---
TOCTitle: 'MS13-039'
Title: 'Microsoft Security Bulletin MS13-039 - 重要'
ms:assetid: 'ms13-039'
ms:contentKeyID: 61238854
ms:date: '04/18/2014'
ms:mtpsurl: 'https://technet.microsoft.com/zh-TW/library/ms13-039(v=Security.10)'
---

Security Bulletin

Microsoft Security Bulletin MS13-039 - 重要
===========================================

HTTP.sys 中的資訊安全風險可能會允許阻斷服務 (DoS) (2829254)
-----------------------------------------------------------
...
此資訊安全更新可解決 Microsoft Windows 中一項未公開報告的資訊安全風險。如果攻擊者傳送蓄意製作的 HTTP 封包到受影響的 Windows 伺服器或用戶端，則此資訊安全風險可能會允許阻斷服務 (DoS)。

對於受支援版本的 Windows 8 與 Windows Server 2012，此資訊安全更新的等級為「重要」。如需更多資訊，請參閱本節中**＜受影響及不受影響的軟體＞**小節。

此資訊安全更新會修正 HTTP.sys 處理特定 HTTP 標頭的方式，藉以解決此資訊安全風險。如需更多有關此資訊安全風險的資訊，請參閱下節**＜資訊安全風險資訊＞**下的特定資訊安全風險項目的＜常見問題集 (FAQ)＞小節。
...
| 知識庫文件     | [2829254](https://support.microsoft.com/kb/2829254) |
|----------------|-----------------------------------------------------|
| 檔案資訊       | 是                                                  |
| SHA1/SHA2 雜湊 | 是                                                  |
| 已知問題       | 是                                                  |
...
[適用於 32 位元系統的 Windows 8](http://www.microsoft.com/downloads/details.aspx?familyid=a6a65ec6-cfdd-458c-835b-f8c7a803f1df)  
(2829254)
</td>
<td style="border:1px solid black;">
阻斷服務 (DoS)
...
<td style="border:1px solid black;">
[適用於 64 位元系統的 Windows 8](http://www.microsoft.com/downloads/details.aspx?familyid=db3fa389-b1f1-4431-aafa-0354416a521a)  
(2829254)
</td>
<td style="border:1px solid black;">
阻斷服務 (DoS)
...
<tr>
<td style="border:1px solid black;">
[Windows Server 2012](http://www.microsoft.com/downloads/details.aspx?familyid=53b182bc-700a-4c55-81fd-2d4eb910badf) (Server Core 安裝)  
(2829254)
</td>
<td style="border:1px solid black;">
阻斷服務 (DoS)
...
| 適用於 Itanium 型系統的 Microsoft Windows Server 2003 SP 2                   |
...
<th style="border:1px solid black;" >
HTTP.sys 阻斷服務 (DoS) 資訊安全風險 - CVE-2013-1305
</th>
<th style="border:1px solid black;" >
彙總嚴重性等級
</th>
...
當 HTTP 通訊協定堆疊 (HTTP.sys) 不當處理惡意 HTTP 標頭時，Windows Server 2012 及 Windows 8 中即存在阻斷服務 (DoS) 資訊安全風險。成功利用此資訊安全風險的攻擊者可傳送蓄意製作的 HTTP 標頭到受影響的 Windows 伺服器或用戶端，藉以在 HTTP 通訊協定堆疊中觸發無窮迴圈。

若要以一般性資訊安全風險清單中的標準項目來檢視此資訊安全風險，請參閱 [CVE-2013-1305](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2013-1305) (英文)。
...
-   **在企業外圍防火牆封鎖 TCP 連接埠 80 和 443**

    TCP 連接埠 80 和 443 用於起始與受影響元件的連線。在網路周邊防火牆封鎖 TCP 連接埠 80 與 443 有助於保護防火牆後方的系統免遭此資訊安全風險的攻擊。此操作有助於保護網路免遭來自企業外的攻擊。封鎖企業外圍受影響的連接埠是免遭網際網路攻擊的最佳防禦措施。但系統仍可能會受到來自其企業外圍內的攻擊影響。

    **因應措施的影響。**將無法從外部網路存取 HTTP 和 HTTPS 應用程式。

    **如何復原因應措施**。在企業外圍防火牆取消封鎖 TCP 連接埠 80 和 443。如需更多關於連接埠的資訊，請參閱 TechNet 文章：[TCP 和 UDP 連接埠的指派](http://technet.microsoft.com/library/cc977599) (英文)。
...
**造成這項資訊安全風險的原因為何？**   
HTTP 通訊協定堆疊 (HTTP.sys) 不當處理惡意的 HTTP 標頭，因而導致此資訊安全風險。

**攻擊者可能會利用此資訊安全風險採取什麼行動？**   
成功利用此資訊安全風險的攻擊者會造成系統停止回應。

**攻擊者如何利用此資訊安全風險？**   
在 HTTP 攻擊案例中，攻擊者可能會傳送蓄意製作的 HTTP 封包到受影響的系統，觸發資訊安全風險。

**因為此資訊安全風險而承受風險的主要系統有哪些？**   
受此資訊安全風險影響的系統主要是執行 Windows Server 2012 的網際網路對向部署系統。

**更新的作用何在？**   
此更新會修正 HTTP.sys 處理特定 HTTP 標頭的方式，藉以解決此資訊安全風險。
...
<td style="border:1px solid black;">請參閱 <a href="http://support.microsoft.com/kb/2829254?ln=zh-tw">Microsoft 知識庫文件編號 2829254</a></td>
</tr>
<tr class="odd">
...
| **檔案資訊**     | 請參閱 [Microsoft 知識庫文件編號 2829254](http://support.microsoft.com/kb/2829254?ln=zh-tw)                                              |
...
-   V1.1 (2013 年 12 月 19 日)： 修訂公告，宣佈一項偵測變更，以修正 Windows RT (2829254) 更新提供程序的問題。此變更僅屬偵測變更。更新檔案沒有變更。已成功安裝本更新的客戶不必採取任何行動。
