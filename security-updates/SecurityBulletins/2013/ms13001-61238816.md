---
TOCTitle: 'MS13-001'
Title: 'Microsoft Security Bulletin MS13-001 - 重大'
ms:assetid: 'ms13-001'
ms:contentKeyID: 61238816
ms:date: '04/18/2014'
ms:mtpsurl: 'https://technet.microsoft.com/zh-TW/library/ms13-001(v=Security.10)'
---

Security Bulletin

Microsoft Security Bulletin MS13-001 - 重大
===========================================

Windows 列印多工緩衝處理器元件中的資訊安全風險可能會允許遠端執行程式碼 (2769369)
--------------------------------------------------------------------------------
...
此資訊安全更新可解決 Microsoft Windows 中一項未公開報告的資訊安全風險。如果列印伺服器收到蓄意製作的列印工作，此資訊安全風險可能會允許遠端執行程式碼。最佳實作的防火牆和標準預設防火牆設定有助於防止網路受到來自企業外的攻擊。最佳方式建議連線至網際網路的系統盡可能曝露最少數量的連接埠。

對於所有受支援版本的 Windows 7 和 Windows Server 2008 R2，此資訊安全更新的等級為「重大」。如需詳細資訊，請參閱本節中的＜受影響及不受影響的軟體＞小節。

此資訊安全更新可修正 Windows 列印多工緩衝處理器處理蓄意製作列印工作的方式，藉以解決此項資訊安全風險。如需更多有關此資訊安全風險的資訊，請參閱下節＜資訊安全風險資訊＞下的特定資訊安全風險項目的＜常見問題集 (FAQ)＞小節。
...
(KB2769369)  
(英文下載更新程式)  
[適用於 32 位元系統的 Windows 7](http://www.microsoft.com/downloads/zh-tw/details.aspx?familyid=abbc3f31-e940-4750-acb6-5af477bc8390)  
(KB2769369)  
(中文下載更新程式)
</td>
<td style="border:1px solid black;">
...
(KB2769369)  
(英文下載更新程式)  
[適用於 x64 型系統的 Windows Server 2008 R2](http://www.microsoft.com/downloads/zh-tw/details.aspx?familyid=8de63e96-2822-4e62-af54-bd8d4c6d5c19)  
(KB2769369)  
(中文下載更新程式)
</td>
<td style="border:1px solid black;">
...
(KB2769369)  
(英文下載更新程式)  
[適用於 x64 型系統的 Windows Server 2008 R2](http://www.microsoft.com/downloads/zh-tw/details.aspx?familyid=8de63e96-2822-4e62-af54-bd8d4c6d5c19) (Server Core 安裝)  
(KB2769369)  
(中文下載更新程式)
</td>
<td style="border:1px solid black;">
...
**何處可找到資訊安全更新的雜湊？** 
資訊安全更新的 SHA1 和 SHA2 雜湊可用來驗證下載之資訊安全更新套件的真實性。如需此更新的相關雜湊資訊，請參閱 [Microsoft 知識庫文件編號 2769369](https://support.microsoft.com/kb/2769369)。

**我使用的是** **Windows Server 2008 R2 (Server Core** **安裝)，其列於「受影響的軟體」表。為什麼我沒有收到** **KB2769369** **更新？**  
KB2769369 更新僅提供給執行 Windows Server 2008 R2 Server Core 安裝選項、且已啟用列印伺服器核心角色設定的系統。
...
<th style="border:1px solid black;" >
Windows 列印多工緩衝處理器元件資訊安全風險-CVE-2013-0011
</th>
<th style="border:1px solid black;" >
彙總嚴重性等級
</th>
...
<span></span>
在 Microsoft Windows 列印多工緩衝處理器處理蓄意製作之列印工作的方式中，存在遠端執行程式碼的資訊安全風險。此資訊安全風險可能會損毀記憶體，導致攻擊者得以執行任意程式碼。

若要以一般性資訊安全風險清單中的標準項目來檢視此資訊安全風險，請參閱 [CVE-2013-0011](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2013-0011)。

#### Windows 列印多工緩衝處理器元件資訊安全風險 - CVE-2013-0011 的緩和因素

緩和因素是指存在於預設狀態中的設定、共用設定或一般最佳作法，可能會減少資訊安全風險影響的嚴重性。下列緩和因素可能對您的狀況有所助益：

-   最佳實作的防火牆和標準預設防火牆設定有助於防止網路受到來自企業外的攻擊。最佳方式建議連線至網際網路的系統盡可能曝露最少數量的連接埠。

#### Windows 列印多工緩衝處理器元件資訊安全風險 - CVE-2013-0011 的因應措施
...
-   **停用列印多工緩衝處理器服務**
    ...
    **警告：**如果使用「登錄編輯程式」的方式錯誤，可能造成嚴重問題，以致於您必須重新安裝作業系統。Microsoft 無法保證您可以解決因為不正確使用 \[登錄編輯程式\] 所造成的問題。請自行承擔使用 \[登錄編輯程式\] 的風險。
    ...
    2.  在「登錄編輯程式」中，請依序展開到下列位置：`HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\services\`
    ...
    **使用管理部署指令碼：**
    ...
    2.  建立名稱為是 Disable\_spooler.reg 的文字檔，且要包含下列內容：`Windows Registry Editor Version 5.00[HKEY_LOCAL_MACHINE\SYSTEM\CurrentControl``Set\Services\Spooler]"Start"=dword:00000004`
    3.  使用下列命令，在目標系統上執行上述步驟 2 所建立的登錄指令碼：`Regedit /s Disable_spooler.reg`
...
#### Windows 列印多工緩衝處理器元件資訊安全風險 - CVE-2013-0011 的常見問題集

**此資訊安全風險的範圍為何？** 
這是遠端執行程式碼的資訊安全風險。

**造成此資訊安全風險的原因為何？** 
當 Windows 列印多工緩衝處理器無法處理蓄意製作的列印工作時，就會導致此資訊安全風險。
...
**攻擊者如何利用此資訊安全風險？** 
未經驗證的遠端攻擊者可將蓄意製作的列印工作傳送給列印伺服器，進而利用此資訊安全風險。

**因為此資訊安全風險而承受風險的主要系統有哪些？** 
工作站與伺服器最可能受這個資訊安全風險影響。

**更新的作用何在？** 
此更新可修正 Windows 列印多工緩衝處理器處理蓄意製作列印工作的方式，藉以解決此項資訊安全風險。
...
<td style="border:1px solid black;">請參閱 <a href="http://support.microsoft.com/kb/2769369">Microsoft 知識庫文件編號 2769369</a></td>
</tr>
<tr class="odd">
