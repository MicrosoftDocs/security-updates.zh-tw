...
**已知問題。** [Microsoft 知識庫文件編號 2769327](http://support.microsoft.com/kb/2769327?ln=zh-tw) 中記載目前已知客戶安裝此資訊安全更新時可能會發生的問題。該文件並說明了解決這些問題的建議解決方案。
...
**何處可找到資訊安全更新的雜湊？**   
資訊安全更新的 SHA1 和 SHA2 雜湊可用來驗證下載之資訊安全更新套件的真實性。如需此更新的相關雜湊資訊，請參閱 [Microsoft 知識庫文件編號 2769327](http://support.microsoft.com/kb/2769327?ln=zh-tw)。
...
若要以一般性資訊安全風險清單中的標準項目來檢視此資訊安全風險，請參閱 [CVE-2013-0005](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2013-0005)。
...
-   **在防火牆封鎖 OData Web 應用程式連接埠**
    ...
    OData 連接埠通常位在 TCP 連接埠 80 與 443，但也可能位在其他非標準連接埠上。評估您的環境，並且封鎖企業防火牆上主控 OData 服務之所有連接埠的輸入與輸出連線，將有助於保護系統防範嘗試利用這個資訊安全風險的攻擊。建議您封鎖所有網際網路中來路不明的輸入通訊，以防他人利用其他連接埠攻擊系統。如需更多關於連接埠的資訊，請參閱 TechNet 文章：[TCP 和 UDP 連接埠的指派](http://go.microsoft.com/fwlink/?linkid=21312) (英文)。

-   **用戶端連線至 IIS 時必須驗證**

    您可以啟用 IIS 的驗證，阻止未經驗證的攻擊者利用此資訊安全風險。如需在您的環境中實作驗證的作法相關資訊，請參閱您 IIS 版本的 TechNet 文件。
    若是 IIS7，請參閱[在 IIS 7 中設定驗證](http://technet.microsoft.com/en-us/library/cc733010(v=ws.10).aspx)

    若是 IIS6.0，請參閱 [IIS6.0 的整合式 Windows 驗證](http://technet.microsoft.com/en-us/library/cc738016(v=ws.10).aspx)
...
**什麼是 OData？**   
Open Data (OData) 通訊協定是用於查詢和更新資料的網路通訊協定，能讓您解除鎖定資料並讓資料脫離現今應用程式中存在的結構。OData 是在 [HTTP](http://www.w3.org/protocols/)、[Atom Publishing 通訊協定](http://www.ietf.org/rfc/rfc4287.txt) (AtomPub) 與 [JSON](http://json.org/) 等 Web 技術上套用與建立，使您能存取多種應用程式、服務和儲存區的資訊。此通訊協定的出現是來自於過去數年間在多種產品內實作 AtomPub 用戶端和伺服器而來的豐富經驗。OData 可用以顯示與存取多種來源的資訊，包括但不限於關聯式資料庫、檔案系統、內容管理系統及傳統網站。如需 OData 規格的詳細資訊，請參閱 [Open Data 通訊協定網站](http://www.odata.org/)。

**什麼是管理 OData IIS 擴充功能 Web 服務？**  
  管理 OData IIS 擴充功能是一種架構，可透過於 IIS 下執行的 OData 型 Web 服務，輕鬆顯示 Windows PowerShell Cmdlet。如需更多資訊，請參閱[建立管理 OData IIS 擴充功能 Web 服務](http://msdn.microsoft.com/en-us/library/windows/desktop/hh880865(v=vs.85).aspx) (英文)。

**什麼是 WCF？**  
  Windows Communication Foundation (WCF) 是 Microsoft 的整合程式撰寫模型，可用於建立服務導向應用程式，讓開發人員製作安全可靠的交易解決方案，以利跨平台整合並與現有投資交互運用。如需 WCF 的詳細資訊，請參閱 [Windows Communication Foundation](http://msdn.microsoft.com/en-us/library/dd456779.aspx) (英文)。
...
<td style="border:1px solid black;">請參閱 <a href="http://support.microsoft.com/kb/2736416?ln=zh-tw">Microsoft 知識庫文件編號 2736416</a><br />
請參閱 <a href="http://support.microsoft.com/kb/2736428?ln=zh-tw">Microsoft 知識庫文件編號 2736428</a></td>
</tr>
<tr class="odd">
...
<tr class="odd">
<td style="border:1px solid black;"><strong>檔案資訊</strong></td>
<td style="border:1px solid black;">請參閱 <a href="http://support.microsoft.com/kb/2736418?ln=zh-tw">Microsoft 知識庫文件編號 2736418</a><br />
請參閱 <a href="http://support.microsoft.com/kb/2736422?ln=zh-tw">Microsoft 知識庫文件編號 2736422</a><br />
請參閱 <a href="http://support.microsoft.com/kb/2736428?ln=zh-tw">Microsoft 知識庫文件編號 2736428</a></td>
</tr>
<tr class="even">
<td style="border:1px solid black;"><strong>登錄機碼驗證</strong></td>
...
<tr class="even">
<td style="border:1px solid black;"><strong>檔案資訊</strong></td>
<td style="border:1px solid black;">請參閱 <a href="http://support.microsoft.com/kb/2736693?ln=zh-tw">Microsoft 知識庫文件編號 2736693</a></td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>登錄機碼驗證</strong></td>
...
<tr class="even">
<td style="border:1px solid black;"><strong>檔案資訊</strong></td>
<td style="border:1px solid black;">請參閱 <a href="http://support.microsoft.com/kb/2736693?ln=zh-tw">Microsoft 知識庫文件編號 2736693</a><br />
請參閱 <a href="http://support.microsoft.com/kb/2753596?ln=zh-tw">Microsoft 知識庫文件編號 2753596</a></td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>登錄機碼驗證</strong></td>
<td style="border:1px solid black;">Microsoft .NET Framework 3.5：<br />
