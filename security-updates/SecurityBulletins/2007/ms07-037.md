---
TOCTitle: 'MS07-037'
Title: 'Microsoft Security Bulletin MS07-037 - 重大'
ms:assetid: 'ms07-037'
ms:contentKeyID: 61238055
ms:date: '04/18/2014'
ms:mtpsurl: 'https://technet.microsoft.com/zh-TW/library/ms07-037(v=Security.10)'
---

Microsoft Security Bulletin MS07-037 - 重大
===========================================

Microsoft Office Publisher 2007 中的弱點可能會允許遠端執行程式碼 (936548)
-------------------------------------------------------------------------
...
這個重要的安全性更新能解決一項公開揭發的弱點。 如果使用者檢視蓄意製作的 Microsoft Office Publisher 檔案，此弱點可能會允許遠端執行程式碼。 系統上帳戶使用者權限較低的使用者，其受影響的程度比擁有系統管理權限的使用者要小。 要利用此項弱點發動攻擊，必須要有使用者的互動才能奏效。

此重要安全性更新適用於受支援版本的 Microsoft Office Publisher 2007。如需更多資訊，請參閱本節中的＜受影響及不受影響的軟體＞小節。

此安全性更新可透過修改 Microsoft Office Publisher 2007 對於格式錯誤 .pub 檔的處理方式，來解決此項弱點。 如需更多有關弱點的資訊，請參閱下節＜弱點資訊＞下的特定弱點項目的＜常見問題集 (FAQ)＞小節。
...
<td style="border:1px solid black;"><a href="https://www.microsoft.com/download/details.aspx?familyid=25d272e7-f2dd-4342-92be-7ebc2e770b44">Microsoft Office Publisher 2007 英文下載更新程式</a>
<a href="https://www.microsoft.com/download/details.aspx?displaylang=zh-tw&amp;familyid=25d272e7-f2dd-4342-92be-7ebc2e770b44">Microsoft Office Publisher 2007 中文下載更新程式</a></td>
...
<th style="border:1px solid black;" >Publisher 無效記憶體參照弱點 – CVE-2007-1754</th>
<th style="border:1px solid black;" >彙總嚴重性等級</th>
</tr>
</thead>
<tbody>
...
Publisher 無效記憶體參照弱點 – CVE-2007-1754  
--------------------------------------------
  
將磁碟的應用程式資料寫入記憶體時，Publisher 未充分清理記憶體資源，這其中存在遠端執行程式碼的弱點。 攻擊者可蓄意製作 Publisher (.pub) 頁面以利用此弱點。 當使用者檢視 .pub 頁面時，此弱點可能會允許遠端執行程式碼。 成功利用此弱點的攻擊者可以取得受影響系統的完整控制權。
  
若要以一般性弱點清單中的標準項目來檢視此弱點，請參閱 [CVE-2007-1754](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2007-1754)。
  
#### Publisher 無效記憶體參照弱點 – CVE-2007-1754 的緩和因素
...
-   在開啟來自網際網路或限制的網站區域的文件之前，2007 Microsoft Office System 會先顯示有 \[開啟\]、\[儲存\] 或 \[取消\] 的提示畫面。  
-   無法透過電子郵件自動攻擊此弱點。 使用者必須順利開啟電子郵件訊息中傳送的附件，攻擊才可進行。
  
#### Publisher 無效記憶體參照弱點 – CVE-2007-1754 的因應措施
  
因應措施指的是無法徹底修正弱點，但有助於在套用更新之前封鎖已知攻擊模式的設定變更。 Microsoft 測試了下列因應措施和狀態，討論因應措施是否會降低功能：
  
-   對於來自於不受信任的來源或在非預期情況下從信任來源收到的 Microsoft Office 檔案，請勿輕易開啟或儲存。 當使用者開啟蓄意製作的檔案時，即可能遭受利用此弱點的攻擊。  
-   修改 pubconv.dll 的存取控制清單
  
    若要將 pubconv.dll 的存取控制清單 (ACL) 修改為較多的限制性，請依照下列步驟執行：
  
1.  按一下 \[開始\]，按一下 \[執行\]，鍵入 cmd，再按一下 \[確定\]。  
2.  請在命令提示字元鍵入下列命令。 請記錄目前檔案的 ACL (包括繼承設定)，以便未來您必須復原此修改時可以參考：  
...
3.  請在命令提示字元鍵入下列命令，以拒絕 'everyone' 群組存取此檔案：  
    ...
    **因應措施的影響：** 需要使用舊版 Publisher 文件的應用程式可能無法再正常運作。
  
#### Publisher 無效記憶體參照弱點 – CVE-2007-1754 的常見問題集
  
**這個弱點的範圍為何？**  
如果成功利用此弱點，這個遠端執行程式碼的弱點可允許攻擊者以登入的使用者身分執行任意程式碼。
  
**造成這個弱點的原因為何？**  
Publisher 未正確驗證從格式錯誤檔案讀取的記憶體值。
...
在網頁攻擊的案例中，攻擊者必須架設網站，其中包含嘗試利用此弱點的 Publisher 檔案。 此外，受侵害的網站以及接受或存放使用者提供之內容的網站裡，也可能包含蓄意製作以利用本弱點的內容。 攻擊者並不能強迫使用者造訪蓄意製作的網站， 而是引誘他們自行前往。一般的做法是設法讓使用者按一下通往攻擊者網站的連結。
  
**因為這個弱點而承受風險的主要系統有哪些？**  
使用 Microsoft Publisher 時所透過的系統風險最高。 若系統管理員允許使用者登入伺服器並執行程式，則伺服器可能遭受更大的風險。 然而，最佳實務強烈建議您制止這種行為。
  
**更新的作用何在？**  
此更新可透過正確驗證從檔案讀取的記憶體值，從而移除此項弱點。
...
|---------------------------------|------------|------------|  
| Microsoft Office Publisher 2007 | 有         | 有         |
...
| **部署**                               |                                                                                                                                               |  
| 在沒有使用者介入的情況下安裝           | publisher2007-kb936646-fullfile-x86-glb.exe /passive                                                                                          |  
| 在不重新啟動的情況下安裝               | publisher2007-kb936646-fullfile-x86-glb.exe /norestart                                                                                        |  
...
針對 Publisher 2007 所有受支援與受影響的版本：
...
感謝 [eEye](https://www.eeye.com/) 回報 Publisher 無效記憶體參照弱點 - CVE-2007-1754。
