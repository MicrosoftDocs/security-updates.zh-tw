---
TOCTitle: 'MS07-006'
Title: 'Microsoft Security Bulletin MS07-006 - 重要'
ms:assetid: 'ms07-006'
ms:contentKeyID: 61237831
ms:date: '04/18/2014'
ms:mtpsurl: 'https://technet.microsoft.com/zh-TW/library/ms07-006(v=Security.10)'
---

Microsoft Security Bulletin MS07-006 - 重要
===========================================
...
-   Microsoft Windows XP Service Pack 2 — [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=f821b3a0-4e5a-4737-b9bf-1249f6683f4d)│[中文下載更新程式](https://www.microsoft.com/download/details.aspx?displaylang=zh-tw&familyid=f821b3a0-4e5a-4737-b9bf-1249f6683f4d)
-   Microsoft Windows XP Professional x64 Edition — [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=75abff9b-c2b5-4151-b366-4be652882944)
-   Microsoft Windows Server 2003 和 Microsoft Windows Server 2003 Service Pack 1 — [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=418acc52-0ebd-4623-81a7-5eacc21c3965)│[中文下載更新程式](https://www.microsoft.com/download/details.aspx?displaylang=zh-tw&familyid=418acc52-0ebd-4623-81a7-5eacc21c3965)
-   Microsoft Windows Server 2003 for Itanium-based Systems 及 Microsoft Windows Server 2003 SP1 for Itanium-based Systems — [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=dc33a2fc-2d01-4577-b133-017493d1f278)
-   Microsoft Windows Server 2003 x64 Edition — [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=c3e55066-b34e-485d-ac04-179f8e3a407a)
...
| 弱點識別碼                                                                                                               | 弱點的影響 | Windows XP Service Pack 2 | Windows Server 2003 | Windows Server 2003 Service Pack 1 |
|--------------------------------------------------------------------------------------------------------------------------|------------|---------------------------|---------------------|------------------------------------|
| Windows Shell Hardware Detection 弱點 - [CVE-2007-0211](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2007-0211) | 權限提高   | 重要                      | 重要                | 重要                               |

此項[評估](https://technet.microsoft.com/security/bulletin/rating)的根據包括：受弱點影響的系統類型、系統的一般部署模式，以及弱點遭利用後對系統所造成的影響。

**注意：**Windows Server 2003 與 Windows Server 2003 Service Pack 1 之安全性更新亦適用於 Windows Server 2003 R2。

**注意：**非 x86 作業系統版本對應到 x86 作業系統版本的嚴重性等級如下：

-   Windows XP Professional x64 Edition 嚴重性等級與 Windows XP Professional Service Pack 2 相同。
...
|-------------------------------------------------------------------------|-----------------------------|---------------------------|---------------------|------------------------------------|
| [**MS06-045**](https://technet.microsoft.com/security/bulletin/ms06-045) | 不適用                      | 取代                      | 取代                | 取代                               |
...
#### Windows Shell Hardware Detection 弱點 - CVE-2007-0211：

作業系統執行新硬體的偵測與註冊作業的方式中，存在 Windows Shell [權限提高](https://go.microsoft.com/fwlink/?linkid=21142)的弱點。 這項弱點可能讓經過驗證的使用者取得完整控制權。

#### Windows Shell Hardware Detection 弱點 - CVE-2007-0211 的緩和因素：

-   攻擊者必須擁有有效的登入認證，才能利用這項弱點。 匿名使用者無法利用這個弱點。
-   在 Windows XP Service Pack 2 和 Windows Server 2003 Service Pack 1 上，攻擊者必須擁有系統管理員權限，才能從遠端利用這項弱點。

#### Windows Shell Hardware Detection 弱點 - CVE-2007-0211 的因應措施：

Microsoft 已經測試過以下的因應措施。 雖然這些因應措施不能徹底解決弱點，但有助於封鎖已知的攻擊行為。 如果因應措施會降低功能，以下小節將會描述功能降低的情況。

-   **停用 Shell Hardware Detection 服務**  

    停用 Shell Hardware Detection 服務有助於保護受影響的系統免遭此弱點的攻擊。 若要停用 Shell Hardware Detection 服務，請依照下列步驟進行：

    1.  按一下 \[開始\]，再按 \[控制台\]。或是，指向 \[設定\]，然後按一下 \[控制台\]。
    2.  按兩下 \[系統管理工具\]。
    3.  按兩下 \[服務\]。
    4.  按兩下 \[Shell Hardware Detection\] 服務。
    5.  在 \[啟動類型\] 清單中，按一下 \[已停用\]。
    6.  按一下 \[停止\]，然後按一下 \[確定\]。

    您也可以在命令提示字元下使用下列命令來停止及停用 Shell Hardware Detection 服務：

    `sc stop ShellHWDetection & sc config ShellHWDetection start= disabled`

    **因應措施的影響：** 如果您停用 **Shell Hardware Detection** 服務，則可能無法利用「快速切換使用者」功能。 因此，我們建議您只在不需要與數位影像處理裝置通訊的系統上使用此因應措施。

#### Windows Shell Hardware Detection 弱點 - CVE-2007-0211 的常見問題集：

**這個弱點的範圍為何？**  
這是[權限提高](https://go.microsoft.com/fwlink/?linkid=21142)弱點。 成功利用此弱點的攻擊者可以取得受影響系統的完整控制權。 攻擊者接下來將能安裝程式，檢視、變更或刪除資料，或建立具有完整使用者權限的新帳戶。 攻擊者必須擁有有效的系統登入認證並執行程式，才能利用這項弱點。

**什麼是 Shell Hardware Detection 服務？**  
Shell Hardware Detection 服務會提供自動播放硬體事件的通知。

**造成這個弱點的原因為何？**  
Microsoft Windows 的硬體初始化和偵測功能中的一個函數參數未經過驗證。

**攻擊者可能會利用這項弱點採取什麼行動？**  
成功利用此弱點的攻擊者可以取得受影響系統的完整控制權。

**什麼人可以利用此弱點？**  
攻擊者必須擁有有效的系統登入認證並執行程式，才能利用這項弱點。

**攻擊者如何利用這項弱點？**  
如果要利用這項弱點，攻擊者必須先通過系統驗證並執行蓄意製作的應用程式。 攻擊者接著可強制受影響的系統執行硬體偵測和初始化事件。 可從本機發出此要求，或透過終端機服務工作階段發出要求，方法是呼叫蓄意製作的應用程式、硬體裝置，或可利用此弱點並取得受影響系統的完整控制權的存放磁碟區。

**因為這個弱點而承受風險的主要系統有哪些？**  
工作站和終端機伺服器的風險最高。 若系統管理員允許使用者登入伺服器並執行程式，則伺服器可能遭受更大的風險。 然而，最佳實務強烈建議您制止這種行為。

**是否可以透過網際網路利用這個弱點？**  
否。 攻擊者必須能夠登入要攻擊的特定系統才行。 未經驗證的攻擊者無法利用此弱點從遠端載入及執行程式。

**更新的作用何在？**  
此更新程式會驗證受影響函式的參數，進而移除弱點。
...
**此弱點與 MS06-045 所修正的弱點有何關係？**  
這兩個弱點都存在於 Windows Shell 中，而這個更新可解決一項 [MS06-045](https://technet.microsoft.com/security/bulletin/ms06-045) 尚未涵蓋的弱點。MS06-045 能完全保護該公告中所提及的弱點，但是它並沒有對這個新弱點提供解決之道。 此更新程式會取代 MS06-045。您必須安裝此更新程式，讓系統不受這兩個弱點的影響。
...
**注意：**使用 **/quiet** 參數將會隱藏所有訊息。 這包括隱藏失敗訊息。 當系統管理員使用 **/quiet** 參數時，則應該採取妥當的做法驗證是否已順利安裝。 當系統管理員使用這個參數時，也應檢閱 KB928255.log 檔案，查看是否有任何失敗訊息。
...
系統管理員也可以使用 Spuninst.exe 公用程式來移除此安全性更新。 Spuninst.exe 公用程式位於 %Windir%\\$NTUninstallKB928255$\\Spuninst 資料夾中。
