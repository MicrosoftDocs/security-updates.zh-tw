...
|--------------------------------------------------------------------------------------------------|----------------|---------------------------|---------------------|------------|---------------------|
| Kerberos 弱點 - [CAN-2005-1981](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=can-2005-1981) | 拒絕服務       | 無                        | 中度                | 無         | 中度                |
| PKINIT 弱點 - [CAN-2005-1982](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=can-2005-1982)   | 資訊洩露及偽造 | 輕微                      | 輕微                | 輕微       | 輕微                |
...
是。 除了本公告＜弱點詳細資訊＞中列出的變更外，這個更新還包含下列功能上的變更： 提供的安全性更新可解決由外部回報的安全性弱點。 另外新增了可選用 (但建議採用) 的登錄機碼 RequireAsChecksum，以加強保護，協助防範其他潛在的 PKINIT 相關弱點。 此登錄機碼在 [Microsoft 知識庫文件編號 904766](https://support.microsoft.com/kb/904766) 中說明。REG\_DWORD 登錄值位於下列登錄機碼中：
...
是。 MBSA 2.0 能判斷是否需要此更新程式。 MBSA 2.0 能偵測 Microsoft Update 支援產品的安全性更新。 如需關於 MBSA 的詳細資訊，請造訪 [MBSA 網站](https://go.microsoft.com/fwlink/?linkid=21134)。
**注意**：在網域環境中，網域控制站與網域用戶端必須都安裝安全性更新，以協助防範 PKINIT 弱點。
...
存在[拒絕服務](https://go.microsoft.com/fwlink/?linkid=21142)弱點，可能會允許攻擊者向 Windows 網域控制站傳送蓄意製作的訊息，從而導致 Active Directory 網域中驗證使用者的服務停止回應。
...
**這個弱點的範圍為何？**  
這是一項[拒絕服務](https://go.microsoft.com/fwlink/?linkid=21142)的弱點。 成功利用此弱點的攻擊者會使伺服器自動重新啟動，並在這段時間內阻止伺服器回應驗證要求。 此弱點存在於執行網域控制站角色的伺服器，例如 Windows 2000 Server 或 Window Server 2003。對用戶端造成的唯一影響是，如果其網域控制站停止回應，用戶端可能無法登入網域。
...
Windows 2000 及更新版作業系統將 Kerberos 用作預設驗證通訊協定。 Kerberos 可提供安全的使用者驗證。 此外，由於 Kerberos 是一種業界標準，因此還允許交互操作。" Active Directory 網域控制站會維護使用者帳戶與登入資訊，以為 Kerberos 服務提供支援。 例如，Kerberos 是一種用來存取 [Active Directory](https://go.microsoft.com/fwlink/?linkid=21200) 中資料的通訊協定。 如需有關 Kerberos 的詳細資訊，請造訪 [Kerberos 驗證說明網站](https://www.microsoft.com/technet/prodtechnol/windows2000serv/maintain/security/kerberos.mspx)或 [TechNet 網站](https://www.microsoft.com/technet/archive/security/news/kerb2000.mspx)。
...
-   攻擊者所用的帳戶以及此攻擊目標所用的帳戶必須可讓其帳戶用於智慧卡驗證。 如需有關在企業中啟用智慧卡所需步驟的詳細資訊，請造訪下列[網站](https://www.microsoft.com/technet/prodtechnol/windowsserver2003/library/serverhelp/b989f4fd-febd-42e1-a130-9e0f33800734.mspx)。
...
**什麼是 PKINT？**  
PKINIT 是適用於「Kerberos 初始驗證之公開金鑰密碼技術」的一種網際網路工程工作小組 (IETF) 網際網路草案。 Windows 2000 及更新版本使用 IETF「Kerberos 初始驗證之公開金鑰密碼技術」網際網路草案的草案 9。 當您使用智慧卡進行互動式登入時，Windows 會用到此通訊協定。 IETF 網際網路草案可在下列 [IETF 網站](https://www.ietf.org)取得。
...
-   [Shell, Inc.](https://www.shell.com) 的 [Tony Chin](mailto:tony.chin@shell.com) 回報 Kerberos 弱點 (CAN-2005-1981)。
