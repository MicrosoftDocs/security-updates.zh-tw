---
TOCTitle: 'MS05-046'
Title: 'Microsoft Security Bulletin MS05-046 - 重要'
ms:assetid: 'ms05-046'
ms:contentKeyID: 61237425
ms:date: '04/18/2014'
ms:mtpsurl: 'https://technet.microsoft.com/zh-TW/library/ms05-046(v=Security.10)'
---

Microsoft Security Bulletin MS05-046 - 重要
===========================================

Client Service for NetWare 中的弱點可能會允許遠端執行程式碼 (899589)
--------------------------------------------------------------------
...
**應該閱讀此文件的對象：** 使用 Client Service for NetWare 或 Gateway Service for NetWare 的客戶
...
-   Microsoft Windows 2000 Service Pack 4 – [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=261a7d4d-90fc-4529-9c4a-b630196c6a83)
-   Microsoft Windows XP Service Pack 1 和 Microsoft Windows XP Service Pack 2 – [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=4c1c2c16-99e7-4701-a3f8-65b803b8b881)
-   Microsoft Windows Server 2003 和 Microsoft Windows Server 2003 Service Pack 1 – [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=8ab86ba3-54cd-44d7-8016-de6e3ed51021)
...
-   [適用於 Netware 的 Windows 服務](https://www.microsoft.com/windows2000/sfn/default.mspx)
...
這個更新程式能解決一項新發現且未公開報告的弱點。 遠端執行程式碼的弱點會存在於 Client Service for NetWare (CSNW) 中。 依照預設，CSNW 並未安裝於任何受影響的作業系統版本上。 唯有手動安裝 CSNW 的客戶，才會受此問題的影響。 本公告的＜弱點詳細資訊＞部分會提供這項弱點的相關資訊。 此項服務亦稱為 Gateway Service for NetWare on Windows 2000 Server。
...
| 弱點識別碼                                                                                                         | 弱點的影響     | Windows 2000 | Windows XP | Windows Server 2003 | Windows Server 2003 Service Pack 1 |
|--------------------------------------------------------------------------------------------------------------------|----------------|--------------|------------|---------------------|------------------------------------|
| Client Service for NetWare 弱點 - [CAN-2005-1985](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=can-2005-1985) | 遠端執行程式碼 | 重要         | 重要       | 重要                | 重要                               |
...
**為何即使受影響的服務未安裝，Windows Update 與 Microsoft Update 仍提供此項安全性更新，且 MBSA 建議此更新？**  
在某些情況下，負責支援受影響服務的受影響檔案，被預設為作業系統的一部分。 即使這些檔案存在於系統中，除非您已安裝受影響的服務，否則系統不會受到此問題的影響。 但是，由於日後可能會根據已複製到本機系統中的檔案來啟用這些服務，因此，Windows Update 與 Microsoft Update 將為這些系統提供此安全性更新，而且 MBSA 也會判定這些系統需要更新程式。 這樣能讓 SMS 將這些安全性更新部署到用戶端，以免日後用戶端安裝受影響的服務，而可能含有此弱點。 **注意：**此更新不適用於 Windows XP Home Edition。 Windows XP Home Edition 並未含有受影響的元件。
...
是。 SMS 能協助偵測及部署本安全性更新。 如需關於 SMS 的詳細資訊，請造訪 [SMS 網站](https://www.microsoft.com/taiwan/smserver/)。 若要偵測 Microsoft Windows 及其他受影響的 Microsoft 產品，需要使用安全性更新盤點工具 (Security Update Inventory Tool)。 如需更多關於安全性更新盤點工具 (Security Update Inventory Tool) 的資訊，請造訪下列 [SMS 網站](https://www.microsoft.com/taiwan/smserver/)。 如需更多關於安全性更新盤點工具 (Security Update Inventory Tool) 的使用限制資訊，請參閱 [Microsoft 知識庫文件編號 306460](https://support.microsoft.com/kb/306460)。您可以使用 SMS 的[目錄與軟體發佈](https://go.microsoft.com/fwlink/?linkid=33333)功能以部署此更新。
...
#### Client Service for NetWare 弱點 - CAN-2005-1985

Client Service for NetWare (CSNW) 中存在的遠端執行程式碼弱點，可以讓成功利用此弱點的攻擊者取得受影響系統的完整控制權。

#### Client Service for NetWare 弱點的緩和因素 - CAN-2005-1985：
...
-   若 CSNW 為手動安裝於 Windows Server 2003 Service Pack 1 上，攻擊者必須擁有有效的登入認證，並能夠登入本機，才能利用這項弱點。 在此作業系統版本中，匿名或遠端使用者均無法利用此弱點。
-   對於需要受影響元件的客戶，最佳實作的防火牆和標準預設防火牆設定有助於防止網路受到來自企業外的攻擊。 最佳方式建議連線至網際網路的系統盡可能曝露最少數量的連接埠。

#### Client Service for NetWare 弱點的因應措施 - CAN-2005-1985：

Microsoft 已經測試過以下的因應措施。 這些因應措施並不能徹底解決弱點，但是有助於封鎖已知的攻擊行為。 如果因應措施會降低功能，以下小節將會描述功能降低的情況。

**注意：**CSNW 常與網際網路封包交換 (IPX) 和循序封包交換 (SPX) 等通訊協定有關聯。 但是，CSNW 可以透過任何已安裝的通訊協定，而被利用。 因為 TCP/IP 是最廣為使用的通訊協定，因此下列的因應措施是以 TCP/IP 為根據。 若 IPX 和 SPX 等通訊協定也在使用中，您也應該針對這些通訊協定封鎖適當的連接埠。 如需更多關於 IPX 和 SPX 的資訊，請造訪下列 [Microsoft 網站](https://microsoft.com/resources/documentation/windows/xp/all/reskit/en-us/prch_cnn_goue.asp) (英文)。

-   **如果不需要使用 Client Service for NetWare，請將其移除。**

    如果不需要使用 Client Service for NetWare，請將其移除。 請依照下列步驟執行。 這些步驟僅適用於 Windows XP。 至於其他的作業系統版本，請遵循說明文件上的程序。
...
-   **在防火牆中封鎖 TCP 連接埠 139 和 445：**
    ...
    上述連接埠用於起始與受影響元件的連線。 在企業防火牆封鎖上述連接埠 (含輸入及輸出) 有助於保護防火牆後的系統免遭此弱點的影響。 建議您封鎖所有網際網路中來路不明的輸入通訊，以防他人利用其他連接埠攻擊系統。 如需更多關於連接埠的資訊，請造訪這個[網站](https://go.microsoft.com/fwlink/?linkid=21312)。

-   **為了防範利用此弱點的網路攻擊，請使用個人防火牆，如 Windows XP 及 Windows Server 2003 中提供的**[**網際網路連線防火牆**](https://go.microsoft.com/fwlink/?linkid=33335)**。**

    依照預設，Windows XP 及 Windows Server 2003 中的網際網路連線防火牆功能會封鎖來路不明的輸入網路流量，藉此協助保護網際網路連線。 我們建議您封鎖所有來路不明的網際網路輸入通訊。 在 Windows XP Service Pack 2 和 Windows Server 2003 中，此功能稱為「Windows 防火牆」。
...
#### Client Service for NetWare 弱點的常見問題集 - CAN-2005-1985：

**這個弱點的範圍為何？**  

遠端執行程式碼的弱點會存在於 Client Service for NetWare (CSNW) 中。 成功利用此弱點的攻擊者可以從遠端取得受影響系統的完整控制權。 攻擊者接下來將能安裝程式，檢視、變更或刪除資料，或建立具有完整使用者權限的新帳戶。 此項服務亦稱為 Gateway Service for NetWare on Windows 2000 Server。

**造成這個弱點的原因為何？**  
Client Service for NetWare 中存在未檢查的緩衝區。

**什麼是 Client Service for NetWare？**  
Client Service for NetWare (CSNW) 能讓用戶端存取 NetWare 檔案、列印與目錄服務。 Microsoft 和 Novell 均針對此功能，提供用戶端服務： 分別為 Microsoft Client Service for NetWare 與適用於 Novell Client for Microsoft® Windows XP。 此弱點會影響 Microsoft Client Service for NetWare。 如需有關 NetWare 存取的詳細資訊，請造訪下列[網站](https://www.microsoft.com/resources/documentation/windows/xp/all/reskit/en-us/default.asp?url=/resources/documentation/windows/xp/all/reskit/en-us/prch_cnn_actp.asp)。 此項服務亦稱為 Gateway Service for NetWare on Windows 2000 Server。

**攻擊者可能會利用這項弱點採取什麼行動？**  
成功利用此弱點的攻擊者可以取得受影響系統的完整控制權。

**什麼人可以利用此弱點？**  
任何能將蓄意製作的網路訊息傳遞給受影響系統的匿名使用者，都可以嘗試利用此弱點。 若 CSNW 為手動安裝於 Windows Server 2003 Service Pack 1 上，攻擊者必須擁有有效的登入認證，並能夠登入本機，才能利用這項弱點。 在 Windows Server 2003 Service Pack 1 中，匿名或遠端使用者無法利用這個弱點。

**攻擊者如何利用這項弱點？**  
攻擊者可以建立一系列蓄意製作的網路訊息，並將訊息傳送給受影響的系統，直接在網路上利用這項弱點。 訊息即可使受影響的系統執行程式碼。

**因為這個弱點而承受風險的主要系統有哪些？**  
所有已安裝此 Client Service for NetWare (亦稱為 Gateway Service for NetWare) 的系統，均易遭受此弱點的影響。 依照預設，此元件並未安裝於任何受影響的作業系統版本上。 唯有手動安裝此元件的客戶，才有可能受此問題的影響。
...
**注意：**使用 **/quiet** 參數將會隱藏所有訊息。 這包括隱藏失敗訊息。 當系統管理員使用 **/quiet** 參數時，則應該採取妥當的做法驗證是否已順利安裝。 使用這個參數時，也必須檢視 KB899589.log 檔案，查看是否有失敗訊息。
...
系統管理員也可以使用 Spuninst.exe 公用程式來移除此安全性更新。 Spuninst.exe 公用程式位於 %Windir%\\$NTUninstallKB899589$\\Spuninst 資料夾中。
...
-   **登錄機碼驗證**
    ...
    **注意：**此登錄機碼可能未包含完整的安裝檔案清單。 此外，當系統管理員或 OEM 將 899589 安全性更新整合或匯集到 Windows 安裝原始檔時，可能未正確建立這個登錄機碼。
...
-   **登錄機碼驗證**
    ...
    **注意：**這些登錄機碼可能未包含完整的安裝檔案清單。 此外，當系統管理員或 OEM 將 899589 安全性更新整合或匯集到 Windows 安裝原始檔時，可能未正確建立這些登錄機碼。
...
-   CERT RENATER 的 [Kostya Kortchinsky](https://technet.microsoft.com/zh-TW/mailto:kostya.kortchinsky@renater.fr) 回報 Client Service for NetWare 弱點 (CAN-2005-1985)。
