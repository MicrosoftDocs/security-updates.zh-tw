...
<td style="border:1px solid black;">
[Adobe Flash Player](https://www.microsoft.com/zh-tw/download/details.aspx?id=52274)  
(3167685)
...
<td style="border:1px solid black;">
[Adobe Flash Player](https://www.microsoft.com/zh-tw/download/details.aspx?id=52275)  
(3167685)
...
<td style="border:1px solid black;">
[Adobe Flash Player](https://www.microsoft.com/zh-tw/download/details.aspx?id=52272)  
(3167685)
...
Windows 10 更新中的<sup>[2]</sup> Adobe Flash Player 更新可透過 [Windows Update](https://update.microsoft.com/microsoftupdate/v6/vistadefault.aspx?ln=zh-tw) 或透過 [Microsoft Update Catalog](https://catalog.update.microsoft.com/v7/site/home.aspx).取得。

**注意**  Windows Server 2016 Technical Preview 5 受到影響；彙總嚴重性等級為「重大」，影響為「中等」，為遠端執行程式碼。建議執行本作業系統的客戶透過 [Windows Update](https://update.microsoft.com/microsoftupdate/v6/vistadefault.aspx?ln=zh-tw) 套用可用的更新。
...
-   根據預設，Windows Server 2012 和 Windows Server 2012 R2 會以[增強式安全性設定](https://technet.microsoft.com/zh-tw/library/dd883248.aspx)執行 Internet Explorer。此模式可協助降低在 Internet Explorer 中 Adobe Flash Player 弱點遭到利用的風險。
...
-   **防止 Adobe Flash Player 執行**
    ...
    **警告** 不當使用「登錄編輯程式」可能會導致嚴重的問題，甚至必須重新安裝作業系統。Microsoft 不保證可以解決您不當使用「登錄編輯程式」所導致的問題。請自行承擔使用「登錄編輯程式」的一切風險。
    ...
    2.  按兩下.reg 檔案以套用至個別系統。
        您也可以使用「群組原則」，將此因應措施跨網域套用。如需有關「群組原則」的詳細資訊，請參閱 TechNet 文件：[群組原則集合](https://technet.microsoft.com/zh-tw/library/cc784165(ws.10).aspx)。
...
-   **透過群組原則防止 Adobe Flash Player 執行**
    ...
    **注意** 「群組原則」MMC 嵌入式管理單元可用於設定電腦、組織單位或整個網域的功能。如需有關「群組原則」的詳細資訊，請造訪下列 Microsoft 網站：
    ...
    [核心群組原則工具及設定 (英文)](https://technet.microsoft.com/zh-tw/library/cc784165(v=ws.10).aspx)
...
-   **防止 Adobe Flash Player 在受影系統中的 Office 2010 執行**
    ...
    **警告** 不當使用「登錄編輯程式」可能會導致嚴重的問題，甚至必須重新安裝作業系統。Microsoft 不保證可以解決您不當使用「登錄編輯程式」所導致的問題。請自行承擔使用「登錄編輯程式」的一切風險。
    ...
    可防止控制項在 Internet Explorer 中執行的詳細步驟，請參閱[Microsoft 知識庫文章 240797](https://technet.microsoft.com/zh-tw/library/cc179076.aspx)。請執行文章中的步驟，以在登錄中建立「相容性旗標」值，防止 COM 物件在 Internet Explorer 中初始化。
