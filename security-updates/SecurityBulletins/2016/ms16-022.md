---
TOCTitle: 'MS16-022'
Title: 'Microsoft 資訊安全公告 MS16-022 - 重大'
ms:assetid: 'ms16-022'
ms:contentKeyID: 72239076
ms:date: '05/27/2016'
ms:mtpsurl: 'https://technet.microsoft.com/zh-TW/library/ms16-022(v=Security.10)'
---

Microsoft 資訊安全公告 MS16-022 - 重大
======================================

Adobe Flash Player 的安全性更新 (3135782)
-----------------------------------------

發行日期：2016 年 2 月 9 日 | 更新日期：2016 年 2 月 12 日
...
此安全性更新可解決安裝於 Windows Server 2012、Windows 8.1、Windows Server 2012 R2、Windows RT 8.1、Windows 10 和 Windows 10 1511 版上之 Adobe Flash Player 中的弱點。如需詳細資訊，請參閱**＜受影響的軟體＞**一節。此更新可更新 Internet Explorer 10、Internet Explorer 11 和 Microsoft Edge 中受影響的 Adobe Flash 程式庫，以解決 Adobe Flash Player 中的弱點。

<span id="KBArticle"></span>
如需有關此更新的詳細資訊，請參閱 [Microsoft 知識庫文章編號 3135782](https://support.microsoft.com/zh-cn/kb/3135782)。

弱點資訊
--------

<span id="sectionToggle1"></span>
此安全性更新可解決 Adobe 資訊安全公告 [APSB16-04](https://helpx.adobe.com/cn/security/products/flash-player/apsb16-04.html) 中描述的以下弱點。
...
下列軟體版號或版本會受到影響。未列出的版號或版本不是超出支援週期就是不受影響。若要瞭解您的軟體版本的支援週期，請參閱 [Microsoft 產品技術支援週期網站](https://support.microsoft.com/zh-cn/lifecycle)。
...
<sup>[1]</sup>此更新僅透過 [Windows Update](https://update.microsoft.com/microsoftupdate/v6/vistadefault.aspx?ln=zh-cn) 提供。

<sup>[2]</sup>適用於 Windows 10 更新的 Adobe FlashPlayer 更新可透過 [Windows Update](https://update.microsoft.com/microsoftupdate/v6/vistadefault.aspx?ln=zh-cn) 或 [Microsoft Update Catalog](https://catalog.update.microsoft.com/v7/site/home.aspx) 取得。

**注意** Windows Server Technical Preview 4 已受到影響，彙總嚴重性等級為「重大」，而影響等級為「中度」(遠端執行程式碼)。建議執行此作業系統的客戶套用更新，您可以透過 [Windows Update](https://update.microsoft.com/microsoftupdate/v6/vistadefault.aspx?ln=zh-cn) 取得更新。
...
-   依照預設，Windows Server 2012 和 Windows Server 2012 R2 上的 Internet Explorer 會以稱為[增強式資訊安全設定](https://technet.microsoft.com/zh-cn/library/dd883248.aspx)的受限制模式執行。此模式有助於降低在 Internet Explorer 中 Adobe Flash Player 弱點遭到利用的可能性。
...
-   **避免執行 Adobe Flash Player**
    ...
    **警告：**如果使用「登錄編輯程式」的方式錯誤，可能造成嚴重問題，以致於您必須重新安裝作業系統。Microsoft 不保證可以解決您不當使用「登錄編輯程式」所導致的問題。請自行承擔使用 \[登錄編輯程式\] 的風險。
    ...
    2.  連按兩下 .reg 檔案，將檔案套用至個別的系統。
        您也可以使用群組原則將檔案套用到整個網域。如需更多關於「群組原則」的資訊，請參閱 TechNet 文章：[群組原則集合](https://technet.microsoft.com/zh-cn/library/cc784165(ws.10).aspx)。
...
-   **使用群組原則避免在 Internet Explorer 中執行 Adobe Flash Player**
    ...
    **注意：** 群組原則 MMC 嵌入式管理單元可用來設定單一機器、單一組織單位，或是整個網域的原則。如需有關「群組原則」的詳細資訊，請造訪下列 Microsoft 網站：
    ...
    [群組原則概觀](https://technet.microsoft.com/zh-cn/library/hh831791)

    [什麼是群組原則物件編輯器？](https://technet.microsoft.com/zh-cn/library/cc737816(v=ws.10).aspx)

    [核心群組原則工具及設定](https://technet.microsoft.com/zh-cn/library/cc784165(v=ws.10).aspx)
...
-   **在受影響系統的 Office 2010 中停用 Adobe Flash Player**
    ...
    **警告：**如果使用「登錄編輯程式」的方式錯誤，可能造成嚴重問題，以致於您必須重新安裝作業系統。Microsoft 不保證可以解決您不當使用「登錄編輯程式」所導致的問題。請自行承擔使用 \[登錄編輯程式\] 的風險。
    ...
    如需避免控制項在 Internet Explorer 中執行的詳細步驟，請參閱 [Microsoft 知識庫文件編號 240797](https://technet.microsoft.com/zh-cn/library/cc179076.aspx)。按照文章中的步驟，在登錄中建立 Compatibility Flags 值，以避免在 Internet Explorer 中產生 COM 物件。
...
Microsoft 了解資訊安全業界所做的努力，其盡責地透露弱點來協助我們保護客戶。請參閱[致謝](https://technet.microsoft.com/zh-cn/library/security/mt674627.aspx) (英文) 以取得詳細資訊。
...
-   V1.1 (2016 年 2 月 12 日)：修訂公告，在下載中心新增更新 3135782 的連結，該更新適用於 32 位元系統的 Windows 8.1、x64 型系統的 Windows 8.1、Windows Server 2012 及 Windows Server 2012 R2。這只是資訊的變更。已成功安裝此更新的客戶不必採取任何進一步的行動。

*頁面產生時間：2016-02-12 11:06-08:00。*
