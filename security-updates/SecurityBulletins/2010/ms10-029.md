...
-   **ISATAP 用戶端上的可能路由器清單 (PRL)：** PRL 可讓用戶端針對連入的 ISATAP 通道封包，進行來源位址檢查。 安裝此安全性更新後，預設會啟用 PRL，此功能有助於解決 CVE-2010-0812 所說明的 ISATAP IPv6 來源位址偽造弱點。您可在 [Microsoft 知識庫文件編號 978338](http://support.microsoft.com/kb/978338/zh-tw) 中找到 PRL 的詳細資訊。  
-   **ISATAP 介面上的「鄰近伺服器無法連線偵測功能」(NUD)：** NUD 的作用是驗證鄰近的伺服器是否可連線。 在傳送 ISATAP 封包之前，ISATAP 節點會執行 NUD。 預設不會啟用 NUD，網路系統管理員必須手動設定 NUD。 如需 NUD 及其啟用方法的詳細資訊，請參閱 [Microsoft 知識庫文件編號 978338](http://support.microsoft.com/kb/978338/zh-tw)。
...
**注意：**雖然預設會在這些平台上啟用 PRL，但不會啟用 NUD，因此必須由網路系統管理員手動設定 NUD。 如需此功能的詳細資訊，請檢閱 [Microsoft 知識庫文件編號 978338](http://support.microsoft.com/kb/978338/zh-tw)。
...
若要在「一般性弱點」清單標準條目中檢視此弱點，請參閱 [CVE-2010-0812](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-0812) (英文)。
...
**什麼是 ISATAP？**    
內部網站自動通道定址通訊協定 (ISATAP) 在 IPv4 內部網路內提供 IPv6 連線功能。 如需 ISATAP 的相關資訊，請參閱 [RFC5214： 內部網站自動通道定址通訊協定 (ISATAP)](http://www.ietf.org/rfc/rfc5214.txt)。
...
<td style="border:1px solid black;">請參閱 <a href="http://support.microsoft.com/kb/978338/zh-tw">Microsoft 知識庫文件編號 978338</a></td>
</tr>
<tr class="even">
