...
**何處可找到檔案資訊詳細資料？**    
檔案資訊詳細資料請參閱 [Microsoft 知識庫文件編號 951071](https://support.microsoft.com/kb/951071)。
...
若要以一般性弱點清單中的標準項目來檢視此弱點，請參閱 [CVE-2008-3479](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2008-3479)。
...
-   **停用訊息佇列服務：**
    ...
    -   根據群組原則：  
        ...
        **注意：**  您也可以參閱《Windows Serve 2003 安全性指南》(英文)。 其中包含了如何停用服務的資訊。  
        ...
        [瞭解群組原則功能集步驟指南 (英文)](https://technet.microsoft.com/en-us/library/bb742376.aspx)
...
**什麼是訊息佇列？**    
Microsoft Message Queuing 技術能讓在不同時間執行的應用程式，跨越可能暫時離線的異質網路及系統進行溝通。 應用程式將訊息傳送到佇列，並讀取來自於佇列的訊息。 訊息佇列提供有保障的訊息傳遞、有效率的路由處理、安全性，和以優先順序為基礎的訊息處理。 它能用於實作非同步及同步訊息處理方式的解決方案。 如需更多關於訊息佇列的資訊，請參閱[訊息佇列產品說明文件](https://www.microsoft.com/windowsserver2003/technologies/msmq/default.mspx)。
  
**什麼是 RPC？**    
Microsoft RPC 乃是在分散式運算環境中用於執行程式設計的模型。 RPC 的目標在於提供透明的通訊方式，進而讓用戶端如同是與伺服器直接進行通訊。 Microsoft 的 RPC 實作相容於開放軟體基金會 (OSF) 分散式運算環境 (DCE) RPC。 如需更多關於 RPC 的資訊，請參閱 [RPC MSDN 網站](https://msdn.microsoft.com/en-us/library/ms691207(vs.85).aspx) (英文)。
...
<td style="border:1px solid black;">請參閱 <a href="https://support.microsoft.com/kb/951071">Microsoft 知識庫文件編號 951071</a></td>
</tr>
<tr class="even">
...
-   感謝 [TippingPoint](https://www.tippingpoint.com/) 以及 [Zero Day Initiative](https://www.zerodayinitiative.com/) 回報 MSMQ 遠端執行程式碼弱點 (CVE-2008-3479)
