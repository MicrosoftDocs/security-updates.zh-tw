...
**何處可找到檔案資訊詳細資料？**      
檔案資訊詳細資料可在 [Microsoft 知識庫文件編號 960714](https://support.microsoft.com/kb/960714) 中找到。
...
若要以一般性弱點清單中的標準項目來檢視此弱點，請參閱 [CVE-2008-4844](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2008-4844)。
...
-   依照預設，Windows Vista 與更新版本上 Internet Explorer 7 和 Internet Explorer 8 Beta 2 的[受保護模式](https://www.microsoft.com/windows/windows-vista/features/ie7-protected-mode.aspx)，可以透過限制啟動另一個程式或未經使用者同意儲存檔案的要求，保護使用者及其系統免受惡意下載的影響。 這包括使用者或系統檔案和設定。
...
-   **停用 XML 資料島功能**
    ...
    **如何復原安全性摘要報告 961051 中所介紹的因應措施**
    ...
    [Microsoft 安全性摘要報告 961051](https://technet.microsoft.com/security/advisory/961051) 針對此因應措施介紹手動登錄變更的方法。 如果您曾經使用這種方法，而現在想要復原此因應措施，請使用下列登錄檔案：
...
Windows Vista 中的 Internet Explorer 7 和 Internet Explorer 8 Beta 2 在網際網路安全性區域中預設會以[受保護模式](https://www.microsoft.com/windows/windows-vista/features/ie7-protected-mode.aspx)執行。 (在內部網路區域中，受保護模式預設為關閉。) 受保護模式會大幅減少攻擊者在使用者的電腦上寫入、變更、摧毀資料或安裝惡意程式碼的能力。 利用 Windows Vista 完整機制以限制存取程序、檔案和具有更高完整層級之登錄機碼的權限。

**什麼是資料執行防止 (DEP)？**  
資料執行防止 (DEP) 包括在 Internet Explorer 內；在 Internet Explorer 7 中預設為停用，而在 Internet Explorer 8 Beta 2 中預設為啟用。DEP 設計用於避免程式碼在標示為不可執行的記憶體上執行，以協助破壞攻擊。 如需更多關於 Internet Explorer 中的 DEP 資訊，請參閱下列張貼： [https://blogs.msdn.com/ie/archive/2008/04/08/ie8-security-part-I\_3A00\_-dep-nx-memory-protection.aspx](https://blogs.msdn.com/ie/archive/2008/04/08/ie8-security-part-i_3a00_-dep-nx-memory-protection.aspx)。最近，已發行概念驗證程式碼，說明略過 DEP 的方法。 但是，這個摘要報告中所包括將安全性滑桿設定為 \[高\] 的因應措施，以及套用其中一項 OLEDB32.dll 因應措施，對於封鎖目前的攻擊仍然有效。

**更新的作用何在？**      
安全性更新會透過修改 Internet Explorer 驗證資料繫結參數，以及導致弱點利用狀況的錯誤所採取的處理方式，進而移除這類弱點。

**當安全性公告發行時，這項弱點是否已揭發出來？**      
是。 這項弱點已經遭到公開揭發。 這項弱點已被指派一般性弱點編號 [CVE-2008-4844](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2008-4844)。
...
<td style="border:1px solid black;">請參閱 <a href="https://support.microsoft.com/kb/960714">Microsoft 知識庫文件編號 960714</a></td>
</tr>
<tr class="even">
