...
<td style="border:1px solid black;"><a href="https://technet.microsoft.com/security/bulletin/ms06-052">MS06-052</a></td>
</tr>
<tr class="even">
...
**何處可找到檔案資訊詳細資料？**    
檔案資訊詳細資料可在 [Microsoft 知識庫文件編號 950762](https://support.microsoft.com/kb/950762) 中找到。
...
若要以一般性弱點清單中的標準項目來檢視此弱點，請參閱 [CVE-2008-1440](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2008-1440)。
...
PGM 是一種可靠且可擴充的通訊協定，能讓接收端偵測資料是否遺失，要求重新傳送遺失的資料，或通知應用程式已發生無法復原的資料遺失。 PGM 是仰賴接收端的通訊協定，這表示接收端必須負責確定是否已收到所有資料，免除傳送端對於通訊可靠性的責任。 若應用程式要求多重來源到多重接收端之間的多點傳送資料傳遞不得重覆，便可以使用 PGM。 PGM 不支援送達確認，也不保證多重傳送端的封包排序。 如需 PGM 的詳細資訊，請參閱下列 [MSDN 文章](https://msdn.microsoft.com/en-us/library/ms740125(vs.85).aspx)。
  
**什麼是 Message Queuing？**  
Microsoft Message Queuing（MSMQ）是 Windows 作業系統的一項元件，可讓合作的應用程式互相傳送和接收訊息；即使預定的收件者應用程式並未執行，或執行寄件者或收件者應用程式的電腦沒有連線到網路。 MSMQ 會儲存和轉寄訊息，直到訊息到達目的地佇列為止。 之後，收件者應用程式執行時，便可從佇列擷取訊息。 MSMQ 會分隔寄件者與收件者應用程式，因此它們不必同時執行。 Message Queuing 提供內建的增強安全性、交易支援和其他功能。 如需 MSMQ 的詳細資訊，請參閱 [MSMQ： 常見問題集](https://www.microsoft.com/windowsserver2003/techinfo/overview/msmqfaq.mspx) (英文)。
...
若要以一般性弱點清單中的標準項目來檢視此弱點，請參閱 [CVE-2008-1441](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2008-1441)。
...
<td style="border:1px solid black;">請參閱 <a href="https://support.microsoft.com/kb/950762">Microsoft 知識庫文件編號 950762</a></td>
</tr>
<tr class="even">
