...
-   Microsoft Windows XP Service Pack 1 和 Microsoft Windows XP Service Pack 2 — [英文下載更新程式](http://www.microsoft.com/downloads/details.aspx?familyid=ce264ac4-6ca3-4732-9016-3143ff1bca2f) │ [中文下載更新程式](http://www.microsoft.com/downloads/details.aspx?displaylang=zh-tw&familyid=ce264ac4-6ca3-4732-9016-3143ff1bca2f)
...
|-------------------------------------------------------------------------------------------------------|----------------|---------------------------|---------------------------|
| PGM 程式碼執行弱點 - [CVE-2006-3442](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2006-3442) | 遠端執行程式碼 | 重要                      | 重要                      |
...
PGM 是一種可靠且可擴充的通訊協定，能讓接收端偵測資料是否遺失，要求重新傳送遺失的資料，或通知應用程式已發生無法復原的資料遺失。 PGM 是仰賴接收端的通訊協定，這表示接收端必須負責確定是否已收到所有資料，免除傳送端確認接收的責任。 若應用程式要求多重來源到多重接收端之間的多點傳送資料傳遞不得重覆，便可以使用 PGM。 PGM 不支援送達確認，也不保證多重傳送端的封包排序。 如需更多關於 PGM 的資訊，請參閱下列 [MSDN 文章](http://msdn.microsoft.com/library/default.asp?url=/library/en-us/winsock/winsock/pgm_senders_and_receivers.asp)。

**什麼是 MSMQ 與 MSMQ 服務？**  
Microsoft Message Queuing (MSMQ) 服務能讓在不同時間執行的應用程式，跨越可能暫時離線的異質網路及系統進行溝通。 如需更多關於 PGM 的資訊，請參閱下列 [MSDN 文章](http://msdn.microsoft.com/library/default.asp?url=/library/en-us/msmq/html/ff917e87-05d5-478f-9430-0f560675ece1.asp)。
