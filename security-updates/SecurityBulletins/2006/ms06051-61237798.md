...
-   Microsoft Windows 2000 Service Pack 4 — [英文下載更新程式](http://www.microsoft.com/downloads/details.aspx?familyid=83e0c6fb-a542-463a-88fd-dc388605a8ae) │ [中文下載更新程式](http://www.microsoft.com/downloads/details.aspx?displaylang=zh-tw&familyid=83e0c6fb-a542-463a-88fd-dc388605a8ae)
-   Microsoft Windows XP Service Pack 1 和 Microsoft Windows XP Service Pack 2 — [英文下載更新程式](http://www.microsoft.com/downloads/details.aspx?familyid=71e96afc-bc4d-4666-998b-49857007e539) │ [中文下載更新程式](http://www.microsoft.com/downloads/details.aspx?displaylang=zh-tw&familyid=71e96afc-bc4d-4666-998b-49857007e539)
-   Microsoft Windows XP Professional x64 Edition — [英文下載更新程式](http://www.microsoft.com/downloads/details.aspx?familyid=a6fe5b95-11d1-42cd-995c-c8cef8a27aef)
-   Microsoft Windows Server 2003 和 Microsoft Windows Server 2003 Service Pack 1 — [英文下載更新程式](http://www.microsoft.com/downloads/details.aspx?familyid=7d0f888b-df35-40df-baff-0bae2b921aef) │ [中文下載更新程式](http://www.microsoft.com/downloads/details.aspx?displaylang=zh-tw&familyid=7d0f888b-df35-40df-baff-0bae2b921aef)
-   Microsoft Windows Server 2003 for Itanium-based Systems 及 Microsoft Windows Server 2003 SP1 for Itanium-based Systems — [英文下載更新程式](http://www.microsoft.com/downloads/details.aspx?familyid=df365b0b-f97f-4df1-9105-d81b68a110eb)
-   Microsoft Windows Server 2003 x64 Edition — [英文下載更新程式](http://www.microsoft.com/downloads/details.aspx?familyid=3f750ad5-5bfb-4421-a316-ae66f0557eac)
...
|-------------------------------------------------------------------------------------------------------------|----------------|--------------|---------------------------|---------------------------|---------------------|------------------------------------|
| 使用者設定檔權限提高弱點 - [CVE-2006-3443](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2006-3443) | 權限提高       | 重要         | 輕微                      | 輕微                      | 輕微                | 輕微                               |
| 未處理例外狀況弱點 - [CVE-2006-3648](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2006-3648)       | 遠端執行程式碼 | 重大         | 重大                      | 重大                      | 重大                | 重大                               |
...
Windows 2000 啟動應用程式的方式含有[權限提高](http://go.microsoft.com/fwlink/?linkid=21142)弱點。 這項弱點可能讓已登入系統的使用者取得完整控制權。
...
-   **將 SafeDllSearchMode 設為 1**
    將登錄中的 SafeDllSearchMode 設為 1，確保 DLL 搜尋路徑順序將系統目錄置於目前使用中的目錄或使用者設定檔目錄之前。 如需關於動態連結程式庫搜尋順序的詳細資訊，請參閱下列 [MSDN 文章](http://msdn.microsoft.com/library/default.asp?url=/library/en-us/dllproc/base/dynamic-link_library_search_order.asp)。
...
**什麼是 DLL？**
DLL 可定義兩種函式： 匯出函式與內部函式。 匯出函式可供其他模組呼叫，也可從定義函式的 DLL 內部進行呼叫。 而內部函式通常是只能從定義函式的 DLL 內部進行呼叫。 雖然 DLL 能匯出資料，但它的資料通常只能供本身的函式使用。 然而，並沒有方法可防止其他模組讀取或寫入該位址。 DLL 提供為應用程式提供模組化的方法，讓它們的功能更易於更新及重覆使用。 當許多應用程式同時使用相同的功能時，DLL 能降低記憶體的負擔，因為雖然每個應用程式所收到 DLL 資料各不相同，但這些應用程式都共用相同的 DLL 程式碼。 如需更多有關 DLL 的資訊，請參閱下列 [MSDN 文章](http://msdn.microsoft.com/library/default.asp?url=/library/en-us/dllproc/base/dynamic_link_libraries.asp)。

**我的 Windows XP 與 Windows 2003 是否會受影響，因為我並沒有看到 SafeDllSearchMode 登錄機碼？**
否。 在 Windows XP Service Pack 1、Windows XP Service Pack 2、Windows Server 2003 及 Windows Server 2003 Service Pack 1 中，作業系統程式碼已預設 SafeDllSearchMode 為 1，因此不受影響。 新增登錄機碼並將其設為 1 以外的值，將會變更預設設定。 如需更多有關 SafeDllSearchMode 設定選項的資訊，請參閱下列 [MSDN 文章](http://msdn.microsoft.com/library/default.asp?url=/library/en-us/dllproc/base/dynamic-link_library_search_order.asp)。
...
-   **停用「我的電腦」區域中的動態指令碼處理**
    ...
    若您想進一步瞭解這項因應措施所修改的值，請參閱下列[網站](http://support.microsoft.com/default.aspx?scid=182569)。 有兩種方式可部署這項因應措施：
    **注意：**不當使用「登錄編輯程式」可能會造成嚴重的問題，甚至可能需要重新安裝您的作業系統。 Microsoft 無法保證能夠順利解決因不當使用「登錄編輯程式」所造成的問題。 請自行承擔使用「登錄編輯程式」的風險。 如需有關如何編輯登錄的資訊，請檢視「登錄編輯程式」(Regedit.exe) 中的＜變更機碼及數值＞說明主題，或是 Regedt32.exe 中的＜新增及刪除登錄中的資訊＞與＜編輯登錄資料＞說明主題。
    **注意：**我們建議您在編輯之前先將之備份。
...
-   感謝 [Canaudit, Inc](http://www.canaudit.com/) 的 [Reed Arvin](mailto:reed@canaudit.com) 回報使用者設定檔權限提高弱點 - CVE-2006-3443
-   感謝 [Leviathan Security Group](http://www.leviathansecurity.com) 的 Matt Miller 與 Ken Johnson 合作回報未處理例外狀況弱點 - CVE-2006-3684
