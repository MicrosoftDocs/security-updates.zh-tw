---
TOCTitle: 'MS06-032'
Title: 'Microsoft Security Bulletin MS06-032 - 重要'
ms:assetid: 'ms06-032'
ms:contentKeyID: 61237779
ms:date: '04/18/2014'
ms:mtpsurl: 'https://technet.microsoft.com/zh-TW/library/ms06-032(v=Security.10)'
---

Microsoft Security Bulletin MS06-032 - 重要
===========================================
...
**警告：** [Microsoft 知識庫文件編號 917953](https://support.microsoft.com/kb/917953) 詳細記錄了目前已知安裝此安全性更新可能會發生的問題。 該文件並說明了解決這些問題的建議解決方案。 如需更多資訊，請參閱 [Microsoft 知識庫文件編號 917953](https://support.microsoft.com/kb/917953)。

**已測試軟體及安全性更新下載位置：**

**受影響的軟體**

-   Microsoft Windows 2000 Service Pack 4 — [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=305e208c-d75c-471b-9e57-30d01e320ad1&displaylang=en)│[中文下載更新程式](https://www.microsoft.com/download/details.aspx?displaylang=zh-tw&familyid=305e208c-d75c-471b-9e57-30d01e320ad1)
-   Microsoft Windows XP Service Pack 1 和 Microsoft Windows XP Service Pack 2 — [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=b62abe8e-4735-4934-a66e-5b957986efbf&displaylang=en)│[中文下載更新程式](https://www.microsoft.com/download/details.aspx?displaylang=zh-tw&familyid=b62abe8e-4735-4934-a66e-5b957986efbf)
-   Microsoft Windows XP Professional x64 Edition — [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=44213900-9082-45dc-b514-31d38717fe89&displaylang=en)
-   Microsoft Windows Server 2003 和 Microsoft Windows Server 2003 Service Pack 1 — [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=ea319c61-b405-41ab-9eee-d5b3488b90e0&displaylang=en)│[中文下載更新程式](https://www.microsoft.com/download/details.aspx?displaylang=zh-tw&familyid=ea319c61-b405-41ab-9eee-d5b3488b90e0)
-   Microsoft Windows Server 2003 for Itanium-based Systems 及 Microsoft Windows Server 2003 SP1 for Itanium-based Systems — [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=cd8b88b5-f90f-4c0c-a5ad-3641751381c9&displaylang=en)
-   Microsoft Windows Server 2003 x64 Edition — [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=cd8699bc-6760-4f0e-b8e0-2e7d89092ce8)
...
|----------------------------------------------------------------------------------------------------|----------------|--------------|---------------------------|---------------------------|---------------------|------------------------------------|
| IP 來源路由弱點 - [CVE-2006-2379](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2006-2379) | 遠端執行程式碼 | 重要         | 重要                      | 中度                      | 重要                | 中度                               |
...
|--------------|--------------|---------------------------|---------------------------|---------------------|------------------------------------|
| **MS06-007** | 不適用       | 取代                      | 取代                      | 取代                | 取代                               |
| **MS05-019** | 取代         | 未取代                    | 取代                      | 取代                | 不適用                             |
...
<span></span>
#### IP 來源路由弱點 - CVE-2006-2379：

在 TCP/IP 通訊協定驅動程式中有一個遠端執行程式碼弱點，可讓成功利用該弱點之攻擊者取得受影響系統的完整控制權。

#### IP 來源路由弱點 - CVE-2006-2379 的緩和因素：

-   最佳實作的防火牆和標準預設防火牆設定有助於防止網路受到來自企業外的攻擊。 最佳方式建議連線至網際網路的系統盡可能曝露最少數量的連接埠。
-   只有在啟用 IP 來源路由的情況下，系統才有風險。 在 Windows XP Service Pack 2 及 Windows Server 2003 Service Pack 1 等系統上，IP 來源路由是停用的。
-   啟用 Routing and Remote Access 服務就會啟用 IP 來源路由。  不過，Routing and Remote Access 在任何受影響的作業系統版本上都是停用的。
-   嘗試利用此弱點進行的攻擊，其最可能的結果是導致拒絕服務狀態。 但是，無法遠端執行程式碼。

#### IP 來源路由弱點 - CVE-2006-2379 的因應措施：

Microsoft 已經測試過以下的因應措施。 雖然這些因應措施不能徹底解決弱點，但有助於封鎖已知的攻擊行為。 如果因應措施會降低功能，以下小節將會描述功能降低的情況。

-   **於防火牆封鎖包含 IP 來源路由選項 131 及 137 的 IP 封包：**

    包含 IP 來源路由選項 131 及 137 的 IP 網路封包會被用來與受影響的元件進行連線。 在防火牆或路由器上，符合上述條件的網路封包將有助於保護防火牆或路由器後方的系統免遭此弱點的攻擊。 我們建議您封鎖所有來路不明的網際網路輸入通訊。 使用 ISA Server 2004 即可封鎖受影響的傳輸類型。
...
-   **停用 IP 來源路由**
    停用 IP 來源路由功能可使受影響的主機不再處理與 IP 來源有關的封包，因此攻擊者就無法利用這些封包來執行程式碼。 下列步驟可停用 IP 來源路由處理：
    ...
    **注意：**我們建議您在編輯登錄之前先將之備份。
    ...
    3.  新增 DWORD 值： **DisableIPSourceRouting**。 將值設定為 **2**。此值代表停用 IP 來源路由處理。 依照預設，並不存在此機碼。
    4.  若要使變更產生效用，您必須重新啟動系統。

如需更多關於停用 IP 來源路由的資訊，請參閱 MSDN 文章＜How To：強化 TCP/IP 堆疊＞中所提供的「其他防護措施」資訊。 [](https://msdn.microsoft.com/library/default.asp?url=/library/en-us/dnnetsec/html/hthardtcp.asp)

#### IP 來源路由弱點 - CVE-2006-2379 的常見問題集：

**這個弱點的範圍為何？**  
這是遠端執行程式碼的弱點。 成功利用此弱點的攻擊者可以從遠端取得受影響系統的完整控制權。 攻擊者接下來將能安裝程式，檢視、變更或刪除資料，或建立具有完整使用者權限的新帳戶。

**造成這個弱點的原因為何？**  
TCP/IP 通訊協定驅動程式中未檢查的緩衝區。

**什麼是 IP 來源路由？**  
IP 來源路由是一種機制，可讓發送者決定資料包在網路上使用的 IP 路由。
...
**因為這個弱點而承受風險的主要系統有哪些？**  
承受風險的主要對象為已啟用 Routing and Remote Access 的電腦。
...
**更新的作用何在？**  
更新程式會修改在將訊息傳遞至分配的緩衝區之前，TCP/IP 通訊協定驅動程式驗證訊息長度的方式，以消除此項弱點。
...
**注意：**使用 **/quiet** 參數將會隱藏所有訊息。 這包括隱藏失敗訊息。 當系統管理員使用 **/quiet** 參數時，則應該採取妥當的做法驗證是否已順利安裝。 當系統管理員使用這個參數時，也應檢閱 KB917953.log 檔案，查看是否有任何失敗訊息。
...
系統管理員也可以使用 Spuninst.exe 公用程式來移除此安全性更新。 Spuninst.exe 公用程式位於 %Windir%\\$NTUninstallKB917953$\\Spuninst 資料夾中。
...
-   **登錄機碼驗證**
    ...
    **注意：**此登錄機碼可能未包含完整的安裝檔案清單。 此外，當系統管理員或 OEM 將 917953 安全性更新整合或匯集到 Windows 安裝原始檔時，可能未正確建立這個登錄機碼。
...
-   **登錄機碼驗證**
    ...
    **注意：**這些登錄機碼可能未包含完整的安裝檔案清單。 此外，當系統管理員或 OEM 將 917953 安全性更新整合或匯集到 Windows 安裝原始檔時，可能未正確建立這些登錄機碼。
...
-   感謝 [Andrey Minaev](https://technet.microsoft.com/zh-TW/mailto:angel3000@hotbox.ru) 回報 IP 來源路由弱點 (CVE-2006-2379)。
...
**注意：**SMS 使用 Microsoft Baseline Security Analyzer、Microsoft Office Detection Tool 以及企業更新掃描工具，為安全性公告的更新偵測及部署作業提供相當廣泛的支援。 不過這些工具可能無法偵測部分的軟體更新。 在這些情況中，系統管理員可以利用 SMS 的清查功能，判斷特定系統所需要的更新程式。 如需這個程序的詳細資訊，請造訪這個[網站](https://go.microsoft.com/fwlink/?linkid=33341)。 某些安全性更新程式在電腦重新啟動之後，會需要系統管理員的權限。 系統管理員可以用 Elevated Rights Deployment Tool (隨 [SMS 2003 Administration Feature Pack](https://www.microsoft.com/taiwan/smserver/downloads/2003/adminpack.htm) 和 [SMS 2.0 Administration Feature Pack](https://go.microsoft.com/fwlink/?linkid=21161)(英文) 提供來安裝這些更新。
...
-   V1.1 (2006 年 6 月 21 日)： 已更新＜與本安全性更新相關的常見問題集 (FAQ)＞部分，以詳細說明 MS05-019 公告取代資訊。 另已更新＜弱點詳細資料＞部分，以提供更多停用 IP 來源路由的相關資訊。
