---
TOCTitle: 'MS06-051'
Title: 'Microsoft Security Bulletin MS06-051 - 重大'
ms:assetid: 'ms06-051'
ms:contentKeyID: 61237798
ms:date: '04/18/2014'
ms:mtpsurl: 'https://technet.microsoft.com/zh-TW/library/ms06-051(v=Security.10)'
---

Microsoft Security Bulletin MS06-051 - 重大
===========================================

Windows 核心中的弱點可能導致遠端執行程式碼 (917422)
---------------------------------------------------
...
-   Microsoft Windows 2000 Service Pack 4 — [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=83e0c6fb-a542-463a-88fd-dc388605a8ae) │ [中文下載更新程式](https://www.microsoft.com/download/details.aspx?displaylang=zh-tw&familyid=83e0c6fb-a542-463a-88fd-dc388605a8ae)
-   Microsoft Windows XP Service Pack 1 和 Microsoft Windows XP Service Pack 2 — [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=71e96afc-bc4d-4666-998b-49857007e539) │ [中文下載更新程式](https://www.microsoft.com/download/details.aspx?displaylang=zh-tw&familyid=71e96afc-bc4d-4666-998b-49857007e539)
-   Microsoft Windows XP Professional x64 Edition — [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=a6fe5b95-11d1-42cd-995c-c8cef8a27aef)
-   Microsoft Windows Server 2003 和 Microsoft Windows Server 2003 Service Pack 1 — [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=7d0f888b-df35-40df-baff-0bae2b921aef) │ [中文下載更新程式](https://www.microsoft.com/download/details.aspx?displaylang=zh-tw&familyid=7d0f888b-df35-40df-baff-0bae2b921aef)
-   Microsoft Windows Server 2003 for Itanium-based Systems 及 Microsoft Windows Server 2003 SP1 for Itanium-based Systems — [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=df365b0b-f97f-4df1-9105-d81b68a110eb)
-   Microsoft Windows Server 2003 x64 Edition — [英文下載更新程式](https://www.microsoft.com/download/details.aspx?familyid=3f750ad5-5bfb-4421-a316-ae66f0557eac)
...
|-------------------------------------------------------------------------------------------------------------|----------------|--------------|---------------------------|---------------------------|---------------------|------------------------------------|
| 使用者設定檔權限提高弱點 - [CVE-2006-3443](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2006-3443) | 權限提高       | 重要         | 輕微                      | 輕微                      | 輕微                | 輕微                               |
| 未處理例外狀況弱點 - [CVE-2006-3648](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2006-3648)       | 遠端執行程式碼 | 重大         | 重大                      | 重大                      | 重大                | 重大                               |
...
<span></span>
#### 使用者設定檔權限提高弱點 - CVE-2006-3443：

Windows 2000 啟動應用程式的方式含有[權限提高](https://go.microsoft.com/fwlink/?linkid=21142)弱點。 這項弱點可能讓已登入系統的使用者取得完整控制權。

#### 使用者設定檔權限提高弱點 - CVE-2006-3443 的緩和因素：

-   攻擊者必須擁有有效的登入認證，並能夠登入本機，才能利用這項弱點。 匿名或遠端使用者無法利用這個弱點。
-   在 Windows XP Service Pack 1、Windows XP Service Pack 2、Windows Server 2003 及 Windows Server 2003 Service Pack 1 中，SafeDllSearchMode 的預設值為 1，因此不受影響。 若這項預設設定被系統管理員修改或停用，這些系統便可能受此弱點影響。

#### 使用者設定檔權限提高弱點 - CVE-2006-3443 的因應措施：

Microsoft 已經測試過以下的因應措施。 雖然這些因應措施不能徹底解決弱點，但有助於封鎖已知的攻擊行為。 如果因應措施會降低功能，以下小節將會描述功能降低的情況。

-   **將 SafeDllSearchMode 設為 1**
    將登錄中的 SafeDllSearchMode 設為 1，確保 DLL 搜尋路徑順序將系統目錄置於目前使用中的目錄或使用者設定檔目錄之前。 如需關於動態連結程式庫搜尋順序的詳細資訊，請參閱下列 [MSDN 文章](https://msdn.microsoft.com/library/default.asp?url=/library/en-us/dllproc/base/dynamic-link_library_search_order.asp)。
    ...
    **注意：**我們建議您在編輯登錄之前先將之備份。
    ...
    2.  在「登錄編輯程式」中，找到下列登錄機碼： HKEY\_LOCAL\_MACHINE\\SYSTEM\\CurrentControlSet\\ Control\\Session Manager
    3.  新增 DWORD 值： **SafeDllSearchMode**。 將值設為 **1**。這個值會將預設的 DLL 搜尋順序設為優先搜尋系統目錄。 依照預設，在 Windows 2000 中並不存在此機碼。
    4.  若要使變更產生效用，您必須重新啟動系統。

#### 使用者設定檔權限提高弱點 - CVE-2006-3443 的常見問題集：
...
**造成這個弱點的原因為何？**  
在 Windows 2000 中，Winlogon 並不禁止使用無效的路徑。 若將蓄意製作的 DLL 置於使用者目錄中，WinLogon 便可能執行 DLL 的程式碼，導致使用者的權限提高。

**什麼是 Winlogon？**  
Winlogon 是 Windows 中管理安全性相關使用者互動的處理程序。 它負責處理登入與登入要求、機器的鎖定與解除鎖定、密碼變更以及其他多種要求。

**什麼是 DLL？**  
DLL 可定義兩種函式： 匯出函式與內部函式。 匯出函式可供其他模組呼叫，也可從定義函式的 DLL 內部進行呼叫。 而內部函式通常是只能從定義函式的 DLL 內部進行呼叫。 雖然 DLL 能匯出資料，但它的資料通常只能供本身的函式使用。 然而，並沒有方法可防止其他模組讀取或寫入該位址。 DLL 提供為應用程式提供模組化的方法，讓它們的功能更易於更新及重覆使用。 當許多應用程式同時使用相同的功能時，DLL 能降低記憶體的負擔，因為雖然每個應用程式所收到 DLL 資料各不相同，但這些應用程式都共用相同的 DLL 程式碼。 如需更多有關 DLL 的資訊，請參閱下列 [MSDN 文章](https://msdn.microsoft.com/library/default.asp?url=/library/en-us/dllproc/base/dynamic_link_libraries.asp)。

**我的 Windows XP 與 Windows 2003 是否會受影響，因為我並沒有看到 SafeDllSearchMode 登錄機碼？**  
否。 在 Windows XP Service Pack 1、Windows XP Service Pack 2、Windows Server 2003 及 Windows Server 2003 Service Pack 1 中，作業系統程式碼已預設 SafeDllSearchMode 為 1，因此不受影響。 新增登錄機碼並將其設為 1 以外的值，將會變更預設設定。 如需更多有關 SafeDllSearchMode 設定選項的資訊，請參閱下列 [MSDN 文章](https://msdn.microsoft.com/library/default.asp?url=/library/en-us/dllproc/base/dynamic-link_library_search_order.asp)。
...
**攻擊者如何利用這項弱點？**  
如果要利用這項弱點，攻擊者首先必須登入系統。 攻擊者可能在他們的使用者設定檔目錄中放置蓄意製作的 DLL 檔案，然後重新啟動電腦。 攻擊者重新啟動電腦並重新登入之後，DLL 便會執行利用此弱點的程式碼，讓攻擊者得以取得受影響系統的完整權限。
...
**更新的作用何在？**  
此更新程式能防止 WinLogon 在 Windows 2000 中使用無效的路徑，進而移除此項弱點。
...
#### 未處理例外狀況弱點 - CVE-2006-3648：

常駐於記憶體中的數個應用程式管理例外狀況處理的方式含有遠端執行程式碼的弱點。

#### 未處理例外狀況弱點 - CVE-2006-3648 的緩和因素：
...
-   \[限制的網站\] 區域可阻止讀取 HTML 電子郵件時使用動態指令碼與 ActiveX 控制項，有助於減少企圖利用這個弱點的攻擊。 然而，如果使用者按下電子郵件中的連結，仍有可能因為前述之網頁式攻擊而受到此弱點的影響。
-   依照預設，Outlook Express 6、Outlook 2002 和 Outlook 2003 會以 \[限制的網站\] 區域開啟 HTML 電子郵件訊息。 此外，如果已經安裝 Outlook 電子郵件安全性更新程式 (英文)，Outlook 98 和 Outlook 2000 也會以 \[限制的網站\] 區域開啟 HTML 電子郵件訊息。 如果已經安裝 Microsoft 安全性公告 MS04-018，Outlook Express 5.5 Service Pack 2 會在「受限制的網站區域」開啟 HTML 電子郵件訊息。

#### 未處理例外狀況弱點 - CVE-2006-3648 的因應措施：

Microsoft 已經測試過以下的因應措施。 雖然這些因應措施不能徹底解決弱點，但有助於封鎖已知的攻擊行為。 如果因應措施會降低功能，以下小節將會描述功能降低的情況。

-   **停用「我的電腦」區域中的動態指令碼處理**

    若您想進一步瞭解這項因應措施所修改的值，請參閱下列[網站](https://support.microsoft.com/default.aspx?scid=182569)。 有兩種方式可部署這項因應措施：
    **注意：**不當使用「登錄編輯程式」可能會造成嚴重的問題，甚至可能需要重新安裝您的作業系統。 Microsoft 無法保證能夠順利解決因不當使用「登錄編輯程式」所造成的問題。 請自行承擔使用「登錄編輯程式」的風險。 如需有關如何編輯登錄的資訊，請檢視「登錄編輯程式」(Regedit.exe) 中的＜變更機碼及數值＞說明主題，或是 Regedt32.exe 中的＜新增及刪除登錄中的資訊＞與＜編輯登錄資料＞說明主題。
    **注意：**我們建議您在編輯之前先將之備份。

    -   按一下 \[開始\]，按一下 \[執行\]，鍵入 **regedt32**，再按一下 \[確定\]。
    -   在「登錄編輯程式」中，找到下列登錄機碼： HKEY\_CURRENT\_USER\\Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings\\Zones\\0
    -   按兩下 DWORD 值： **1400**。
    -   編輯 DWORD 值對話方塊即出現。 預設值是 0。請將此值變更為 **3**。
    -   關閉任何 Internet Explorer 執行個體。 變更將在 Internet Explorer 下一次啟動時生效。
...
#### 未處理例外狀況弱點 - CVE-2006-3648 的常見問題集：

**這個弱點的範圍為何？**  
這是遠端程式碼的弱點。 成功利用此弱點的攻擊者可以取得受影響系統的完整控制權。 攻擊者接下來將能安裝程式，檢視、變更或刪除資料，或建立具有完整使用者權限的新帳戶。

**造成這個弱點的原因為何？**  
常駐於記憶體中的應用程式未正確處理例外狀況。
...
**因為這個弱點而承受風險的主要系統有哪些？**  
Windows 工作站與伺服器系統的風險最高。 如果系統管理權限不足的使用者被授予登入伺服器並執行程式的能力時，伺服器會面臨更大的風險。 然而，最佳實務強烈建議您制止這種行為。

**是否可以透過網際網路利用這個弱點？**  
是。 攻擊者可以在網際網路上利用這項弱點。

**更新的作用何在？**  
此更新程式會正確卸載鏈結的例外狀況，進而移除此項弱點
...
**注意：**使用 **/quiet** 參數將會隱藏所有訊息。 這包括隱藏失敗訊息。 當系統管理員使用 **/quiet** 參數時，則應該採取妥當的做法驗證是否已順利安裝。 當系統管理員使用這個參數時，也應檢閱 KB917422.log 檔案，查看是否有任何失敗訊息。
...
系統管理員也可以使用 Spuninst.exe 公用程式來移除此安全性更新。 Spuninst.exe 公用程式位於 %Windir%\\$NTUninstallKB917422$\\Spuninst 資料夾中。
...
-   感謝 [Canaudit, Inc](https://www.canaudit.com/) 的 [Reed Arvin](mailto:reed@canaudit.com) 回報使用者設定檔權限提高弱點 - CVE-2006-3443
-   感謝 [Leviathan Security Group](https://www.leviathansecurity.com) 的 Matt Miller 與 Ken Johnson 合作回報未處理例外狀況弱點 - CVE-2006-3684
...
-   [TechNet 更新管理中心 (英文)](https://go.microsoft.com/fwlink/?linkid=69903)
...
-   V1.1 (2006 年 8 月 15 日)： 更新＜安全性更新部分＞中 Windows 2003 的＜檔案資訊＞。
