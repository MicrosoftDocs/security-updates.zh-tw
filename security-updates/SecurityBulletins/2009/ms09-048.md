...
**已知問題。** [Microsoft 知識庫文件編號 967723](http://support.microsoft.com/kb/967723/zh-tw) 中記載了客戶安裝此安全性更新後，可能會發生的目前已知問題。 該文件並說明了解決這些問題的建議解決方案。
...
若要以一般性弱點清單中的標準項目來檢視此弱點，請參閱 [CVE-2008-4609](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2008-4609)。
...
**什麼是 TCP/IP？**    
TCP/IP 是一組網際網路上廣泛使用的網路通訊協定。 網際網路上許多互相連接的網路是由使用不同作業系統和多種硬體架構的電腦所組成，TCP/IP 讓這些網路間能夠互相通訊。 TCP/IP 內含電腦間用來溝通用的標準，以及連接網路和路由傳輸用的慣例。 如需 TCP/IP 的詳細資訊，請參閱 TechNet 文章：[網路和 TCP/IP的概觀](http://go.microsoft.com/fwlink/?linkid=40954) (英文)。
  
**什麼是 TCP 接收視窗大小？**    
傳輸控制通訊協定 (TCP) 接收視窗大小是指在連線的接收端，一次可緩衝的已接收資料量上限 (單位為位元組)。 傳送主機在等待接收主機的認可和更新視窗前，只能傳送該數量的資料。 將 TCP 接收視窗大小設為零可保持連線，但在視窗大小增加前，無法傳輸其餘任何資料位元組。 如需 TCP 接收視窗大小選項的詳細資訊，請參閱 MSDN 文章：[TCP 接收視窗大小和視窗縮放比例](http://msdn.microsoft.com/en-us/library/ms819736.aspx) (英文)。
...
若要以一般性弱點清單中的標準項目來檢視此弱點，請參閱 [CVE-2009-1925](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-1925)。
...
若要以一般性弱點清單中的標準項目來檢視此弱點，請參閱 [CVE-2009-1926](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-1926)。
...
TCP 連線在用戶端與伺服器之間的交易過程中，歷經一連串不同的狀態。 FIN-WAIT-1 狀態表示系統正在等待遠端 TCP 的連線終止要求，或等待先前所傳送的連線終止要求的認可。 FIN-WAIT-2 狀態表示系統正在等待遠端 TCP 的連線終止要求。 如需詳細資訊，請參閱 [RFC 793](http://www.ietf.org/rfc/rfc793.txt)。
...
<td style="border:1px solid black;">請參閱 <a href="http://support.microsoft.com/kb/967723/zh-tw">Microsoft 知識庫文件編號 967723</a></td>
</tr>
<tr class="even">
...
-   感謝 [Outpost24](http://www.outpost24.com/) 的 Jack C. Louis 回報 TCP/IP 零視窗大小弱點 (CVE-2008-4609)  
-   感謝 [Recurity Labs GmbH](http://www.recurity-labs.com/) 的 Felix Lindner 回報 TCP/IP 孤立連線弱點 (CVE-2009-1926)
