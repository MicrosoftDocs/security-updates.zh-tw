---
TOCTitle: 'MS09-064'
Title: 'Microsoft Security Bulletin MS09-064 - 重大'
ms:assetid: 'ms09-064'
ms:contentKeyID: 61238333
ms:date: '04/18/2014'
ms:mtpsurl: 'https://technet.microsoft.com/zh-TW/library/ms09-064(v=Security.10)'
---

Security Bulletin

Microsoft Security Bulletin MS09-064 - 重大
===========================================

License Logging Server 中的弱點可能會允許遠端執行程式碼 (974783)
----------------------------------------------------------------
...
這個資訊安全公告更新可解決 Microsoft Windows 2000 中一項未公開報告的弱點。如果攻擊者傳送蓄意製作的網路訊息至執行 License Logging Server 的電腦，則此弱點可能會允許遠端執行程式碼。 成功利用此弱點的攻擊者可以取得系統的完整控制權。 最佳實作的防火牆和標準預設防火牆設定有助於防止網路受到來自企業外的攻擊。

對於所有受支援版本的 Microsoft Windows 2000，此資訊安全公告更新的等級為「重大」。如需更多資訊，請參閱本節中的＜受影響及不受影響的軟體＞小節。

此資訊安全公告更新可變更 License Logging 服務執行 RPC 封包某特定欄位驗證的方式，藉以解決弱點。 如需更多有關弱點的資訊，請參閱下節＜弱點資訊＞下的特定弱點項目的＜常見問題集 (FAQ)＞小節。
...
<tbody>
<tr class="odd">
<td style="border:1px solid black;"><a href="https://www.microsoft.com/download/details.aspx?familyid=365a8dff-2383-42f6-b567-e545461fd135">Microsoft Windows 2000 Server Service Pack 4 (英文下載更新程式)</a>
<a href="https://www.microsoft.com/download/details.aspx?displaylang=zh-tw&amp;familyid=365a8dff-2383-42f6-b567-e545461fd135">Microsoft Windows 2000 Server Service Pack 4 (中文下載更新程式)</a></td>
...
依照預設，License Logging Server 是否會安裝到 Microsoft Windows 2000 Server？是的，依照預設，Microsoft Windows 2000 Server 會安裝 License Logging 服務，而且會預設啟用。
  
**如何得知我的伺服器上是否使用 License Logging 服務？**   
License Logging 服務在 not available on Microsoft Windows 2000 Professional 上不存在。 依照預設，Microsoft Windows 2000 Server 會安裝且會執行 License Logging 服務。 您可以執行下列程序來判斷是否已安裝 License Logging 服務。
...
<th style="border:1px solid black;" >License Logging Server 堆積溢位弱點 - CVE-2009-2523</th>
<th style="border:1px solid black;" >彙總嚴重性等級</th>
</tr>
</thead>
<tbody>
...
License Logging Server 堆積溢位弱點 - CVE-2009-2523  
---------------------------------------------------
  

Microsoft License Logging Server 軟體處理蓄意製作之 RPC 封包的方式中，存在未經驗證的遠端執行程式碼弱點。 攻擊者不需要驗證即可利用這項弱點，因此可藉由傳送蓄意製作的網路訊息至執行 License Logging 服務的電腦來利用弱點。 成功利用此弱點的攻擊者可以取得系統的完整控制權。
  
若要以一般性弱點清單中的標準項目來檢視此弱點，請參閱 [CVE-2009-2523](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-2523)。
  
#### License Logging Server 堆積溢位弱點 - CVE-2009-2523 的緩和因素
  
緩和因素是指存在於預設狀態中的設定、共用設定或一般最佳作法，可能會減少弱點影響的嚴重性。 下列緩和因素可能對您的狀況有所助益：
  
-   最佳實作的防火牆和標準預設防火牆設定有助於防止網路受到來自企業外的攻擊。 最佳方式建議連線至網際網路的系統盡可能曝露最少數量的連接埠。
  
#### License Logging Server 堆積溢位弱點 - CVE-2009-2523 的因應措施
  
因應措施指的是無法徹底修正弱點，但有助於在套用更新之前封鎖已知攻擊模式的設定變更。 Microsoft 測試了下列因應措施和狀態，討論因應措施是否會降低功能：
  
-   停用 License Logging 服務
  
    停用 License Logging 服務有助於保護您的系統，防止遠端嘗試利用這項弱點。
    ...
    **因應措施的影響** 如果停用 License Logging 服務，任何明確依賴 License Logging 服務的服務便可能在系統事件記錄中記錄錯誤訊息。 如需更多資訊，請參閱 [Microsoft 知識庫文件編號 316631](https://support.microsoft.com/kb/316631/?ln=zh-tw)
...
#### License Logging Server 堆積溢位弱點 - CVE-2009-2523 的常見問題集
  
**這個弱點的範圍為何？**    
這是遠端執行程式碼的弱點。 成功利用此弱點的攻擊者可以取得受影響系統的完整控制權。 攻擊者接下來將能安裝程式，檢視、變更或刪除資料，或建立具有完整使用者權限的新帳戶。
  
**造成這個弱點的原因為何？**    
因為 License Logging 服務從 RPC 呼叫收到字串時並未驗證其長度。 因而導致堆積發生緩衝區溢位。
  
**攻擊者可能會利用這項弱點採取什麼行動？**    
成功利用此弱點的攻擊者可以取得受影響系統的完整控制權。
  
**攻擊者如何利用這項弱點？**    
攻擊者可以建立蓄意製作的 RPC 訊息，並將訊息傳送給受影響的系統，以利用這項弱點。
  
**因為這個弱點而承受風險的主要系統有哪些？**    
受到此弱點影響的系統為執行 License Logging 服務的 Microsoft Windows 2000 Server。 因此，與工作站或其他伺服器相比，網域控制站會受到此弱點影響的風險更大。
  
**更新的作用何在？**    
此資訊安全公告更新會修正 License Logging 服務接收 RPC 訊息時的驗證方式來解決此弱點。
  
什麼是 RPC？ 遠端程序呼叫 (RPC) 是一個通訊協定，程式可將其用來要求網路中其他電腦的程式服務。 RPC 能夠提高互通性，因為使用 RPC 的程式不必解讀支援通訊的網路通訊協定。 在 RPC 中，要求的程式為用戶端而提供服務的程式為伺服器。
...
| **部署**                               |                                                                                                                               |  
| 在沒有使用者介入的情況下安裝           | Windows2000-KB974783-x86-enu /quiet                                                                                           |  
| 在不重新啟動的情況下安裝               | Windows2000-KB974783-x86-enu /norestart                                                                                       |  
| 更新記錄檔                             | KB974783.log                                                                                                                  |  
| 詳細資訊                               | 請參閱＜偵測與部署工具及指南＞小節                                                                                            |  
| **重新開機需求**                       |                                                                                                                               |  
| 需要重新開機？                         | 是，套用此資訊安全公告更新之後，您必須重新啟動系統。                                                                          |  
| HotPatching                            | 不適用                                                                                                                        |  
| **移除資訊**                           | 使用 \[控制台\] 中的 \[新增或移除程式\] 工具，或是 %Windir%\\$NTUninstallKB974783$\\Spuninst 資料夾中的 Spuninst.exe 公用程式 |  
| **檔案資訊**                           | 請參閱 [Microsoft 知識庫文件編號 974783](https://support.microsoft.com/kb/974783)                                              |  
| **登錄機碼驗證**                       | HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Updates\\Windows 2000\\SP5\\KB974783                                               |
...
-   感謝 [TippingPoint DVLabs](https://dvlabs.tippingpoint.com/) 的 Cody Pierce 回報 License Logging Server 堆積溢位弱點 (CVE-2009-2523)
