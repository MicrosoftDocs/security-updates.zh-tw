...
<td style="border:1px solid black;"><a href="https://technet.microsoft.com/security/bulletin/ms07-031">MS07-031</a></td>
</tr>
<tr class="even">
...
**何處可找到檔案資訊詳細資料？**    
如需檔案資訊詳細資料，請參閱 [Microsoft 知識庫文件編號 960225](https://support.microsoft.com/kb/960225)。
...
若要以一般性弱點清單中的標準項目來檢視此弱點，請參閱 [CVE-2009-0085](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-0085)。
...
-   客戶可以實作 Active Directory 憑證對應，這並不會受此弱點影響。 Active Directory 憑證對應可讓具備受信任公開金鑰的使用者，不需輸入使用者名稱及密碼，即可存取網域資源。 用戶端憑證可以直接對應到 Active Directory 使用者帳戶。  
    如需設定憑證對應之不同方式的詳細資訊，請參閱[將憑證對應到使用者帳戶](https://technet.microsoft.com/en-us/library/cc736706.aspx) (英文)。
...
安全通道 (Schannel) 安全性套件是執行安全通訊端層 (SSL) 和傳輸層安全性 (TLS) 網際網路標準驗證通訊協定的安全性支援提供者 (SSP)。 這些元件用於實作安全通訊以支援幾種常見的網際網路及網路應用程式，例如網頁瀏覽。 Schannel 是協助提供驗證服務，以在用戶端和伺服器之間提供安全通訊之安全性套件的一部分。 如需 SChannel 的詳細資訊，請造訪 MSDN 上的 [SChannel](https://msdn.microsoft.com/en-us/library/ms678421.aspx) 網站。
...
這些通訊協定會在伺服器及用戶端系統之間提供加密連線。 當使用者透過網際網路之類的公用網路連線時，SSL 可協助保護資訊。 SSL 支援需要已在伺服器上安裝 SSL 憑證。 如需 SSL 的詳細資訊，請參閱 [XADM： 通訊端層運作的安全程度](https://support.microsoft.com/default.aspx?scid=kb;en-us;q245152) (英文)。
...
<td style="border:1px solid black;">請參閱 <a href="https://support.microsoft.com/kb/960225">Microsoft 知識庫文件編號 960225</a></td>
</tr>
<tr class="even">
...
-   感謝 [Secretaria da Fazenda do Estado do Rio Grande do Sul](https://www.sefaz.rs.gov.br/) 回報 CVE-2009-0085 的 SChannel 偽造弱點 (CVE-2009-0085)  
-   感謝 [Cia de Processamento de Dados do Estado do Rio Grande do Sul](https://www.procergs.rs.gov.br/) 回報 SChannel 偽造弱點 (CVE-2009-0085)
