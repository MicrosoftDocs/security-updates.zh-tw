...
若要以一般性弱點清單中的標準項目來檢視此弱點，請參閱 [CVE-2009-1127](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-1127)。
...
若要以一般性弱點清單中的標準項目來檢視此弱點，請參閱 [CVE-2009-2513](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-2513)。
...
**什麼是 GDI？**    
Microsoft Windows 圖形裝置介面 (GDI) 可讓應用程式在視訊畫面及印表機上使用圖形和格式化文字。 Windows 應用程式不會直接存取圖形硬體。 而是由 GDI 代表應用程式與裝置驅動程式互動。 如需更多關於 GDI 的資訊，請參閱 MSDN 文章 [Windows GDI](http://msdn.microsoft.com/en-us/library/dd145203(vs.85).aspx) (英文)。
...
若要以一般性弱點清單中的標準項目來檢視此弱點，請參閱 [CVE-2009-2514](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-2514)。
...
-   **停用 Internet Explorer 的內嵌字型剖析支援**
    ...
    注意：請參閱 [Microsoft 知識庫文件編號 969947](http://support.microsoft.com/kb/969947/?ln=zh-tw)，瞭解如何使用自動化 Microsoft Fix It 解決方案以啟用或停用此因應措施。
...
-   [核心群組原則工具和設定](http://technet2.microsoft.com/windowsserver/en/library/e926577a-5619-4912-b5d9-e73d4bdc94911033.mspx?mfr=true)
    ...
    **使用管理的部署指令碼**
    ...
    此安全性設定可以手動建立登錄指令碼後，連按兩下匯入登錄機碼，或是在登入或電腦啟動指令碼中一併執行 regedit.exe。 對於受管理的部署，可用 Regedit.exe 的 '-s' 參數，以無提示的方式匯入登錄指令碼。 如需更多關於 regedit 命令列參數的資訊，請參閱 [Microsoft 知識庫文件編號 82821](http://support.microsoft.com/kb/q82821/)：＜登錄機碼編輯器 (REGEDIT) 命令列參數＞。
...
| **檔案資訊**                           | 請參閱 [Microsoft 知識庫文件編號 969947](http://support.microsoft.com/kb/969947/?ln=zh-tw)                                    |  
...
<td style="border:1px solid black;">請參閱 <a href="http://support.microsoft.com/kb/969947/?ln=zh-tw">Microsoft 知識庫文件編號 969947</a></td>
</tr>
<tr class="even">
...
<tr class="odd">
<td style="border:1px solid black;">HotPatching</td>
<td style="border:1px solid black;">本資訊安全公告更新不支援 HotPatching。 如需關於 HotPatching 的詳細資訊，請參閱 <a href="http://support.microsoft.com/kb/897341/?ln=zh-tw">Microsoft 知識庫文件編號 897341</a>。</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"><strong>移除資訊</strong></td>
<td style="border:1px solid black;">使用 [控制台] 中的 [新增或移除程式] 工具，或是 %Windir%\$NTUninstallKB969947$\Spuninst 資料夾中的 Spuninst.exe 公用程式</td>
...
-   感謝 [Google Inc.](http://www.google.com/) 的 Tavis Ormandy 回報 Win32k EOT 剖析弱點 (CVE-2009-2514)
