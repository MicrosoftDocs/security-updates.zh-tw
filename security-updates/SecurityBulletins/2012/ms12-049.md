---
TOCTitle: 'MS12-049'
Title: 'Microsoft Security Bulletin MS12-049 - 重要'
ms:assetid: 'ms12-049'
ms:contentKeyID: 61238781
ms:date: '04/18/2014'
ms:mtpsurl: 'https://technet.microsoft.com/zh-TW/library/ms12-049(v=Security.10)'
---

Security Bulletin

Microsoft Security Bulletin MS12-049 - 重要
===========================================

TLS 中的資訊安全風險可能會導致資訊洩漏 (2655992)
------------------------------------------------

發行: 2012年7月10日 | 更新: 2013年7月11日
...
此資訊安全更新可解決 TLS 中一項公開揭露的資訊安全風險。如果攻擊者攔截受影響系統所服務的加密網站流量，則此資訊安全風險可能會導致資訊洩漏。未使用 CBC 模式的所有加密套件不受影響。

對於所有受支援版本的 Microsoft Windows，此資訊安全更新的等級為「重要」。如需更多資訊，請參閱本節中的＜受影響及不受影響的軟體＞小節。

此安全性更新可修改 Windows 安全通道 (SChannel) 和加密 API 新一代 (CNG) 元件處理加密網路封包的方式， 以解決此資訊安全風險。如需更多有關此資訊安全風險的資訊，請參閱下節＜資訊安全風險資訊＞下的特定資訊安全風險項目的＜常見問題集 (FAQ)＞小節。
...
<td style="border:1px solid black;">
KB2655992 取代 [MS10-049](https://go.microsoft.com/fwlink/?linkid=197104) 中的 KB980436
</td>
</tr>
...
<td style="border:1px solid black;">
KB2655992 取代 [MS12-006](https://go.microsoft.com/fwlink/?linkid=232510) 中的 KB2585542
</td>
</tr>
<tr>
<td style="border:1px solid black;">
...
<td style="border:1px solid black;">
KB2655992 取代 [MS10-085](https://go.microsoft.com/fwlink/?linkid=201705) 中的 KB2207566
</td>
</tr>
<tr>
<td style="border:1px solid black;">
...
<tr>
<td style="border:1px solid black;">
[適用於 32 位元系統的 Windows Server 2008 Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=97030267-6b19-46ae-84ce-0bb1f91ab951) (Server Core 安裝)  
(KB2655992)
</td>
<td style="border:1px solid black;">
資訊洩漏
...
<tr class="alternateRow">
<td style="border:1px solid black;">
[適用於 x64 型系統的 Windows Server 2008 Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=8a5f6e84-5e5b-42c3-a5b7-65defdb0665f) (Server Core 安裝)  
(KB2655992)
</td>
<td style="border:1px solid black;">
資訊洩漏
...
<tr>
<td style="border:1px solid black;">
[適用於 x64 型系統的 Windows Server 2008 R2](https://www.microsoft.com/download/details.aspx?familyid=c9ef728f-abef-4076-bb13-7488af471aa4) (Server Core 安裝)  
(KB2655992)
</td>
<td style="border:1px solid black;">
資訊洩漏
...
<tr class="alternateRow">
<td style="border:1px solid black;">
[適用於 x64 型系統的 Windows Server 2008 R2 Service Pack 1](https://www.microsoft.com/download/details.aspx?familyid=c9ef728f-abef-4076-bb13-7488af471aa4) (Server Core 安裝)  
(KB2655992)
</td>
<td style="border:1px solid black;">
資訊洩漏
...
**何處可找到資訊安全更新的雜湊？**  
資訊安全更新的 SHA1 和 SHA2 雜湊可用來驗證下載之資訊安全更新套件的真實性。如需此更新的相關雜湊資訊，請參閱 [Microsoft 知識庫文件編號 2655992](https://support.microsoft.com/kb/2655992)。
...
TLS 加密通訊協定中存在資訊洩漏資訊安全風險。此資訊安全風險影響的是通訊協定本身，並非 Windows 作業系統所特有。這是一個資訊洩漏資訊安全風險，會允許加密的 TLS 流量遭到解密。由於瀏覽器是主要攻擊媒介，因此這個資訊安全風險主要是影響 HTTPS 流量，且透過 HTTPS 或混合性 HTTP/HTTPS 內容提供服務的所有網站流量都會受到影響。

若要在「一般性資訊安全風險」清單標準條目中檢視此資訊安全風險，請參閱 [CVE-2012-1870](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1870) (英文)。

#### TLS 通訊協定資訊安全風險 - CVE-2012-1870 的緩和因素

緩和因素是指存在於預設狀態中的設定、共用設定或一般最佳作法，可能會減少資訊安全風險影響的嚴重性。下列緩和因素可能對您的狀況有所助益：

-   未使用 CBC 模式的所有加密套件不受影響。

#### TLS 通訊協定資訊安全風險 - CVE-2012-1870 的因應措施

因應措施指的是無法徹底修正資訊安全風險，但有助於在套用更新之前封鎖已知攻擊模式的設定變更。Microsoft 測試了下列因應措施和狀態，討論因應措施是否會降低功能：

-   **在執行Windows Vista、Windows Server 2008、Windows 7或Windows Server 2008 R2之系統上的伺服器軟體中，排定優先處理RC4演算法**

    此攻擊只會影響在 CBC 模式中使用對稱式加密演算法的加密套件，例如 AES，並不影響 RC4 演算法。您可以在伺服器軟體中排定優先處理 RC4 演算法，以促進安全的通訊 (使用 RC4 而非 CBC 模式)。請參閱這篇 MSDN 文章：[排定 Schannel 加密套件優先順序](https://msdn.microsoft.com/en-us/library/bb870930%28v=vs.85%29.aspx) (英文)，瞭解如何執行此作業。

    **警告：**  您與之通訊的用戶端或伺服器必須支援 RC4 演算法。當 RC4 不受支援時，若有其他加密套件可供使用，便會使用該加密套件，而此本因應措施將會無效。

#### TLS 通訊協定資訊安全風險 - CVE-2012-1870 的常見問題集

**此資訊安全風險的範圍為何？**  
這是一個資訊洩漏的資訊安全風險。

**造成此資訊安全風險的原因為何？**  
此資訊安全風險是在使用作業的密碼區塊鏈結 (CBC) 模式時，因 TLS 通訊協定中的設計缺點所造成。

**什麼是TLS和SSL？**  
傳輸層安全性 (TLS) 交握通訊協定負責建立或恢復安全工作階段的必備驗證及金鑰交換。安全通訊端層 (SSL) 是「傳輸層安全性」通訊協定的前身。透過公開金鑰與秘密金鑰技術的結合，TLS 和 SSL 可以執行相同的功能並支援安全的網路通訊。

如需更多資訊，請參閱 TechNet 文章：[TLS/SSL 如何運作](https://technet.microsoft.com/en-us/library/cc783349(ws.10).aspx) (英文)。

**攻擊者可能會利用此資訊安全風險採取什麼行動？**  
成功利用此資訊安全風險來監視加密通訊的攻擊者，可將所攔截的加密流量解密。

**攻擊者如何利用此資訊安全風險？**  
攻擊者可在 HTTP 回應中插入惡意程式碼或是架設蓄意製作且內含惡意程式碼的網站，強制瀏覽器執行此惡意程式碼。此程式碼會在同樣的 TLS 工作階段中，將數個要求傳送到第三方 HTTPS 網站，在該網站中，若有一個先前通過驗證的工作階段存在，便會自動傳送 Cookie。若要利用此資訊安全風險，這是必要的條件。攻擊者必須能攔截此 HTTP 流量，才能在 SSL 中利用此資訊安全風險，從而對部分的加密流量 (例如驗證 Cookie) 進行解密。

**因為此資訊安全風險而承受風險的主要系統有哪些？**  
由於瀏覽器是主要的功擊模式，因此傳送和接收 HTTP 流量的任何系統 (例如工作站或終端機伺服器) 受此資訊安全風險影響的風險最大。

**更新的作用何在？**  
更新會修改 Windows 安全通道 (SChannel) 和加密 API 新一代 (CNG) 元件處理加密網路封包的方式。

**本資訊安全公告發行時，此資訊安全風險是否已揭發出來？**  
是。此資訊安全風險已經遭到公開揭發。這項資訊安全風險已被指派一般性資訊安全風險編號 [CVE-2012-1870](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1870) (英文)。
...
<td style="border:1px solid black;">使用 [控制台] 中的 [新增或移除程式] 項目，或是 %Windir%\$NTUninstallKB2655992$\Spuninst 資料夾中的 Spuninst.exe 公用程式</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>檔案資訊</strong></td>
<td style="border:1px solid black;">請參閱 <a href="https://support.microsoft.com/kb/2655992">Microsoft 知識庫文件編號 2655992</a></td>
</tr>
<tr class="even">
...
-   V1.1 (2013 年 7 月 11 日)： 修訂公告，宣佈一項 2655992 更新的 Windows Vista 套件偵測變更，以修正 Windows Update 在重新提供更新程式方面的問題。此變更僅屬偵測變更。已成功更新系統的客戶不必採取任何行動。
