...
<span></span>
**我已經成功地安裝原始的** **KB2705219** **更新。我需要套用在** **2012** **年** **10** **月** **9** **日重新發行的** **KB2705219** **更新套件嗎？** 
...
**我使用的是** **Windows Server 2008 R2 (Server Core** **安裝)，其列於「受影響的軟體」表。為什麼我**沒有收到這項 **KB2712808** **資訊安全更新**？ 
...
-   **停用** **LanmanWorkstation** **服務**
    ...
    **使用管理部署指令碼**：
    ...
    1.  使用下列管理部署指令碼，建立登錄機碼的備份：`regedit /e lanmanworkstation_configuration_backup.reg HKEY_LOCAL_MACHINE\System\CurrentControlSet\Services\lanmanworkstation`
    2.  將下列文字儲存至副檔名為 .REG 的檔案中 (例如 Disable\_lanmanworkstation.reg)：`Windows Registry Editor Version 5.00[HKEY_LOCAL_MACHINE\System\CurrentControlSet\Services\lanmanworkstation]"Start"=dword:00000004`
...
-   **停用列印多工緩衝處理器服務**
    ...
    **使用管理部署指令碼：**
    ...
    1.  使用下列管理部署指令碼，建立登錄機碼的備份：`regedit /e spooler_configuration_backup.reg ``HKEY_LOCAL_MACHINE\System\CurrentControlSet\Services\spooler`
    2.  建立名稱為是 Disable\_spooler.reg 的文字檔，且要包含下列內容：`Windows Registry Editor Version 5.00[HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\Spooler]"Start"=dword:00000004`
    3.  使用下列命令，在目標電腦上執行上述步驟 2 所建立的登錄指令碼：`R``egedit /s Disable_spooler.reg`
...
-   **停用** **LanmanWorkstation** **服務**
    ...
    **使用管理部署指令碼：**
    ...
    1.  使用下列管理部署指令碼，建立登錄機碼的備份：`regedit /e lanmanworkstation_configuration_backup.reg ``HKEY_LOCAL_MACHINE\System\CurrentControlSet\Services\lanmanworkstation`
    2.  將下列文字儲存至副檔名為 .REG 的檔案中 (例如 Disable\_lanmanworkstation.reg)：`Windows Registry Editor Version 5.00[HKEY_LOCAL_MACHINE\System\CurrentControlSet\Services\lanmanworkstation]"Start"=dword:00000``004`
    3.  使用下列命令，在目標電腦上執行步驟 2 所建立的登錄指令碼：`Regedit /s Disable_lanmanworkstation.reg`
